...
export default class MemoryClient {
  ...
  _validateApiKey(): any {
    if (!this.apiKey) {
      throw new Error("MemB API key is required");
    }
    if (typeof this.apiKey !== "string") {
      throw new Error("MemB API key must be a string");
    }
    if (this.apiKey.trim() === "") {
      throw new Error("MemB API key cannot be empty");
    }
  }

  constructor(options: ClientOptions) {
    this.apiKey = options.apiKey;
    this.host = options.host || "https://api.memb.ai";
  ...
  private async _maybeAliasAnonToEmail(): Promise<void> {
    ...
    try {
      ...
      const sharedAnonId = await getOrCreateMemBUserId();
      const anonIds = await readMemBAnonIds();
      ...
      for (const anonId of candidates) {
        if (seen.has(anonId) || (await isMemBAliased(anonId, email))) continue;
        seen.add(anonId);
        if (await telemetry.captureIdentify(anonId, email)) {
          await markMemBAliased(anonId, email);
        }
      }
    } catch (error: any) {
