/**
 * Best-effort read/write of ~/.memb/config.json from the TS SDK.
 *
...
export async function getOrCreateMemBUserId(): Promise<string | null> {
...
export async function readMemBAnonIds(): Promise<MemBAnonIds | null> {
  const node = await getNodeFs();
  if (!node) return null;
  const config = loadConfig(node);
...
export async function isMemBAliased(
  anonId: string,
  email: string,
): Promise<boolean> {
...
export async function markMemBAliased(
  anonId: string,
  email: string,
): Promise<void> {
  const node = await getNodeFs();
