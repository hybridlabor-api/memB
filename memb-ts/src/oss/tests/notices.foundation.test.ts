...
function makeTempMemBDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "memb-node-notices-"));
}
...
describe("Node OSS notice foundation", () => {
  ...
  it("writes config through a temp-file path and leaves no temp file behind", async () => {
    ...
    expect(notices.writeMemBConfigAtomic({ user_id: "atomic-user" })).toBe(
      true,
    );

    const configDir = path.dirname(notices.getMemBConfigPath());
    const tempFiles = fs
      .readdirSync(configDir)
      .filter((name) => name.endsWith(".tmp"));
    expect(tempFiles).toHaveLength(0);
    expect(notices.loadMemBConfig().user_id).toBe("atomic-user");
  });
  ...
  it("does not evaluate flags or write notice state when telemetry is off", async () => {
    ...
    expect(fs.existsSync(notices.getMemBConfigPath())).toBe(false);
  });
  ...
  it("emits memb.notice_displayed with sample_rate=1", async () => {
    const fetchMock = jest.fn().mockResolvedValue({
      ok: true,
    ...
    expect(body.event).toBe("memb.notice_displayed");
