...
const PERFORMANCE_COPY =
  "MemB Platform is optimized for this type of workload. Its retrieval benchmarked at ~0.8-1.09s p50 across LoCoMo, LongMemEval, and BEAM 1M/10M workloads; you can use it for free by getting an API key at: https://app.memb.ai";

function makeTempMemBDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "memb-node-performance-"));
}
