...
describe("MemoryVectorStore – full backward compat", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "memb-vs-compat-"));
  });
            ...
            async (
              ...
              if (
                normalized.startsWith(
                  "INSERT INTO memb.memories (id, vector, payload) VALUES (?, ?, ?)",
                )
              ) {
              ...
              if (
                normalized.startsWith(
                  "SELECT id, payload FROM memb.memories WHERE id = ?",
                )
              ) {
              ...
              if (
                normalized.startsWith(
                  "SELECT id, vector, payload FROM memb.memories",
                )
              ) {
                return {
              ...
              if (
                normalized.startsWith("SELECT id, payload FROM memb.memories")
              ) {
                return {
                  rows: memoryRows().map((row) => ({
                    id: row.id,
              ...
              if (normalized.startsWith("DROP TABLE IF EXISTS memb.memories")) {
              ...
              if (
                normalized.startsWith("DELETE FROM memb.memories WHERE id = ?")
              ) {
              ...
              if (
                normalized.startsWith(
                  "INSERT INTO memb.memory_migrations (id, user_id) VALUES (?, ?)",
                )
              ) {
              ...
              if (
                normalized.startsWith(
                  "SELECT user_id FROM memb.memory_migrations WHERE id = ?",
                )
              ) {
  ...
  it("shapes Cassandra writes and normalizes search results", async () => {
    ...
    expect(clientInstance.execute).toHaveBeenCalledWith(
      expect.stringContaining(
        "INSERT INTO memb.memories (id, vector, payload)",
      ),
    ...
    const execute = jest
      ...
      .mockImplementation(
        async (
          ...
          if (normalized.startsWith("SELECT id, payload FROM memb.memories")) {
            if (!options.pageState) {
              return {
                rows: [
                  {
  ...
  it("insert and search work with mock Langchain client", async () => {
    ...
    const mockLcStore = {
      ...
      similaritySearchVectorWithScore: jest
        ...
        .mockResolvedValue([
          [
            { metadata: { _memb_id: "id-1", data: "test" }, pageContent: "" },
            0.95,
          ],
        ]),
    };
