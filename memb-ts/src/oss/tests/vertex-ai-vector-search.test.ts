...
describe("VertexAIVectorSearch", () => {
  ...
  it("should exclude the memb-user-id-record sentinel from search results", async () => {
        ...
        nearestNeighbors: [
          {
            neighbors: [
              {
                datapoint: { datapointId: "memb-user-id-record" },
                distance: 0.0,
              },
  ...
  it("should set a user id via the sentinel datapoint", async () => {
    ...
    const request = (store as any).indexClient.upsertDatapoints.mock
      .calls[0][0];
    expect(request.datapoints[0].datapointId).toBe("memb-user-id-record");
  });
        ...
        nearestNeighbors: [
          {
            neighbors: [
              {
                datapoint: {
                  datapointId: "memb-user-id-record",
                  restricts: [
