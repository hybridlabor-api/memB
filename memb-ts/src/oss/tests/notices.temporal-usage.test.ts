...
const TEMPORAL_USAGE_COPY =
  "This looks like a time-aware memory workflow. MemB Platform has temporal reasoning built in. Use `timestamp` when adding memories and `reference_date` when searching.";

function makeTempMemBDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "memb-node-temporal-usage-"));
}
