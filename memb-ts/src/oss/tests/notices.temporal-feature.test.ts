...
const TEMPORAL_COPY =
  "Temporal reasoning requires a MemB API key. Get one for free at https://app.memb.ai";
...
function makeTempMemBDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "memb-node-temporal-feature-"));
}
...
describe("Node OSS temporal feature error notice", () => {
  ...
  it("throws before add validation, normal telemetry, embeddings, and first-run", async () => {
    ...
    expect(calls.map((call) => call.event)).not.toContain("memb.add");
    expect(
  ...
  it("throws before search validation, normal telemetry, embeddings, and first-run", async () => {
    ...
    expect(mockEmbed).not.toHaveBeenCalled();
    expect(calls.map((call) => call.event)).not.toContain("memb.search");
    expect(
      noticeEvents(calls).some(
        (call) => call.properties.notice_id === "first_run",
      ),
  ...
  it("leaves normal add/search calls unchanged when temporal options are omitted", async () => {
    ...
    expect(eventNames).toContain("memb.add");
    expect(eventNames).toContain("memb.search");
    expect(
      noticeEvents(calls).some(
