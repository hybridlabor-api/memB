...
const DECAY_COPY =
  'Memory decay requires MemB Platform. Get a free API key at https://app.memb.ai?utm_source=oss_sdk&utm_medium=in_product&utm_campaign=decay_stub&utm_content=node_error and use `import MemoryClient from "membai"`.';
...
function makeTempMemBDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "memb-node-decay-feature-"));
}
...
function noticeEvents(calls: any[]) {
  return calls.filter((call) => call.event === "memb.notice_displayed");
}
...
describe("Node OSS decay feature error notice", () => {
  let originalMemBDir: string | undefined;
  ...
  it("does not trigger first-run notice or update_project telemetry", async () => {
    ...
    expect(eventNames).toContain("memb.notice_displayed");
    expect(eventNames).not.toContain("memb.update_project");
    expect(
