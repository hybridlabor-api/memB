...
// This index holds Porter-stemmed text, but memb/vector_stores/baidu.py's keyword_search()
...
export class BaiduDB implements VectorStore {
  ...
  // value import would break `import { Memory } from "membai/oss"` for everyone else.
  ...
  private applySchema(preexisting: boolean, schema: TableSchema): void {
    ...
    if (
      ...
      throw new Error(
        `Baidu Mochow table '${label}' exists but is missing the id/data/vector/metadata schema memb requires. Drop it, or point 'tableName' at an unused table.`,
      );
    }
