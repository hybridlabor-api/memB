...
describe("SQLiteManager – directory auto-creation", () => {
  it("creates nested parent directories and writes to the DB", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "memb-sqlite-"));
  ...
  it("end-to-end: mergeConfig + SQLiteManager at configured path", async () => {
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "memb-history-path-"),
    );
...
describe("MemoryVectorStore – path handling", () => {
  ...
  it("uses ~/.memb/vector_store.db by default", () => {
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), "memb-home-"));
    try {
      jest.spyOn(os, "homedir").mockReturnValue(fakeHome);
      new MemoryVectorStore({ dimension: 4 });
      expect(
        fs.existsSync(path.join(fakeHome, ".memb", "vector_store.db")),
      ).toBe(true);
    } finally {
  ...
  it("respects explicit dbPath config", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "memb-vs-"));
  ...
  it("works when CWD is read-only", async () => {
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), "memb-home-"));
    const readOnlyCwd = fs.mkdtempSync(path.join(os.tmpdir(), "memb-ro-"));
  ...
  it("emits migration warning when old CWD-based vector_store.db exists", () => {
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), "memb-home-"));
    const tempCwd = fs.mkdtempSync(path.join(os.tmpdir(), "memb-cwd-"));
...
describe("ensureSQLiteDirectory", () => {
  it("creates nested directories", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "memb-ensure-"));
...
describe("getDefaultVectorStoreDbPath", () => {
  it("returns path under homedir/.memb", () => {
    const result = getDefaultVectorStoreDbPath();
    expect(result).toBe(path.join(os.homedir(), ".memb", "vector_store.db"));
  });
});
