...
export const NOTICE_FLAG_KEY = "memb-oss-notices";
export const NOTICE_EVENT_NAME = "memb.notice_displayed";
...
export function getMemBConfigPath(): string {
  return path.join(getMemBDir(), "config.json");
}

export function loadMemBConfig(): Record<string, any> {
  try {
    const configPath = getMemBConfigPath();
...
export function writeMemBConfigAtomic(config: Record<string, any>): boolean {
  const configPath = getMemBConfigPath();
...
export function recordNoticeOpportunity(
  ...
  const written = writeMemBConfigAtomic(
