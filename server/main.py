...
def _warn_if_unconfigured() -> None:
    ...
    logging.warning(
        ...
        "  Docs: https://docs.memb.ai/open-source/features/rest-api#authentication\n"
        "%s",
        "=" * 72,
        "=" * 72,
    )
...
DEFAULT_LLM_MODEL = os.environ.get("MEMB_DEFAULT_LLM_MODEL", "gpt-4.1-nano-2025-04-14")
DEFAULT_EMBEDDER_MODEL = os.environ.get("MEMB_DEFAULT_EMBEDDER_MODEL", "text-embedding-3-small")
...
class GenerateInstructionsRequest(BaseModel):
    use_case: str = Field(..., description="Description of what the user will use MemB for.")
...
@app.get("/configure", summary="Get current MemB configuration")
...
@app.post("/configure", summary="Configure MemB")
...
def add_memory(memory_create: MemoryCreate, _auth=Depends(verify_auth)):
    ...
    except (ValueError, MemBValidationError) as e:
