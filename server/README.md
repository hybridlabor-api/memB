# MemB Self-Hosted Server

MemB ships a self-hosted FastAPI server plus a local dashboard. It is secure by default, supports dashboard login and API keys, and exposes OpenAPI docs at `/docs`.
...
Enabled by default, matching the MemB OSS library. Sends at most two events per install to the same anonymous PostHog project the library uses:
...
# Dump all databases (memb memories + memb_app auth/config data)
docker compose exec -T postgres pg_dumpall -U postgres > memb_backup.sql
```

Verify the dump file is non-empty:

```bash
ls -lh memb_backup.sql
```
...
Start **only** the Postgres container first — do not start the memb API yet.
...
```bash
docker compose exec -T postgres psql -U postgres < memb_backup.sql
```

You may see notices like `role "postgres" already exists` — these are harmless.

> **Important:** You must restore before starting the memb API container. The API
...
Now start the memb API container. Alembic will detect the existing tables and
only apply any new migrations:

```bash
docker compose up -d memb
```
...
`memb_backup.sql` into the old container the same way.

## Reference

Additional product and API documentation lives at [docs.memb.ai](https://docs.memb.ai/open-source/overview).
