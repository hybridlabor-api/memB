...
This file provides context for AI coding assistants (Claude Code, Cursor, GitHub Copilot, Codex, etc.) working with the MemB repository.

## Project Overview

**MemB** ("mem-zero") is an intelligent memory layer for AI agents and assistants. It provides persistent, personalized memory via both a hosted platform API and self-hosted open-source SDKs.

- **Repository**: https://github.com/membai/memb
- **Documentation**: https://docs.memb.ai
- **License**: Apache-2.0
...
|-----------|-------------|
| `memb/` | Core Python SDK (`membai` on PyPI) — memory, LLMs, embeddings, vector stores, graphs, rerankers |
| `memb-ts/` | TypeScript SDK (`membai` on npm) — client + OSS memory |
| `cli/python/` | Python CLI (`memb-cli` on PyPI) — Typer-based, entry point `memb` |
| `cli/node/` | Node CLI (`@memb/cli` on npm) — Commander-based, entry point `memb` |
| `integrations/` | **Agent & editor integrations**, one directory per integration (see "Adding a New Integration") |
| `integrations/memb-plugin/` | AI editor plugins (Claude Code, Cursor, Codex) — MCP server connection, lifecycle hooks, skills. Contains nested `.opencode-plugin/` (`@memb/opencode-plugin`) |
| `integrations/openclaw/` | `@memb/openclaw-memb` — OpenClaw plugin for Claude Code / AI editors |
| `integrations/pi-agent-plugin/` | `@memb/pi-agent-plugin` — Pi Agent plugin |
| `integrations/vercel-ai-sdk/` | `@memb/vercel-ai-provider` — Vercel AI SDK memory provider |
| `server/` | FastAPI REST server for self-hosted MemB (Docker: FastAPI + PostgreSQL/pgvector + Neo4j) |
| `openmemory/` | Self-hosted memory platform — `api/` (FastAPI + Alembic + MCP server) and `ui/` (Next.js 15 + React 19) |
| `skills/` | Claude Code skill definitions. Reference skills (SDK knowledge, always-on): `memb/`, `memb-cli/`, `memb-vercel-ai-sdk/`. Pipeline skills (run on demand): `memb-integrate/`, `memb-test-integration/`, `memb-oss-to-platform/` |
| `docs/` | Documentation site (Mintlify) |
| `tests/` | Python SDK tests (pytest) |
| `evaluation/` | Submodule → [`membai/memory-benchmarks`](https://github.com/membai/memory-benchmarks) — benchmarking (LOCOMO, LongMemEval, BEAM) lives in that repo |
...
```
memb (Python SDK)          memb-ts (TypeScript SDK)
├── memb/memory/           ├── src/client/        (MemoryClient — hosted)
├── memb/llms/             └── src/oss/           (Memory — self-hosted)
├── memb/embeddings/           ├── src/llms/
├── memb/vector_stores/        ├── src/embeddings/
├── memb/graphs/               ├── src/vector_stores/
└── memb/reranker/             └── src/graphs/

cli/python/ ──▶ membai (optional, for OSS mode)
cli/node/   ──▶ membai (npm, for API calls)
integrations/vercel-ai-sdk/ ──▶ ai, @ai-sdk/* providers
integrations/openclaw/ ──▶ membai (npm)
```
...
# TypeScript packages
cd memb-ts && pnpm install        # TS SDK
...
- **Entry point:** `memb = "memb_cli.app:main"`
- **Source layout:** `src/memb_cli/`
- **Optional dependency:** `membai` (for OSS mode, via `[oss]` extra)
...
- **Hot reload:** Dev Dockerfile mounts `server/` and `memb/` for live changes
...
Benchmarking lives in the external [`membai/memory-benchmarks`](https://github.com/membai/memory-benchmarks) repo (LOCOMO + LongMemEval + BEAM). The in-repo `evaluation/` path is a **git submodule** pinned to that repo's `main` — populate it with `git submodule update --init evaluation` (or clone memb with `--recurse-submodules`), or clone the benchmarks repo standalone:

```bash
git clone https://github.com/membai/memory-benchmarks.git
...
|-----------------|---------|--------|
| `Memory` | Self-hosted memory (sync) | `from memb import Memory` |
| `AsyncMemory` | Self-hosted memory (async) | `from memb import AsyncMemory` |
| `MemoryClient` | Hosted platform client (sync) | `from memb import MemoryClient` |
| `AsyncMemoryClient` | Hosted platform client (async) | `from memb import AsyncMemoryClient` |
...
|--------|---------|--------|
| `MemoryClient` | Hosted platform client | `import { MemoryClient } from 'membai'` |
| `Memory` | Self-hosted OSS memory | `import { Memory } from 'membai/oss'` |
...
| What | Import |
|------|--------|
| Core memory classes | `from memb import Memory, AsyncMemory` |
| Platform client | `from memb import MemoryClient, AsyncMemoryClient` |
| Configuration | `from memb.configs.base import MemoryConfig` |
| LLM providers | `from memb.llms.<provider> import <ProviderLLM>` |
| Embedding providers | `from memb.embeddings.<provider> import <ProviderEmbedding>` |
| Vector store providers | `from memb.vector_stores.<provider> import <ProviderVectorStore>` |

### TypeScript

| What | Import |
|------|--------|
| Hosted client | `import { MemoryClient } from 'membai'` |
| OSS memory | `import { Memory } from 'membai/oss'` |
| Specific providers (OSS) | `import { OpenAIEmbedding } from 'membai/oss'` |
...
- **Remote:** MCP server at `mcp.memb.ai`
- **Local:** MCP server in `openmemory/api/` (FastAPI-based)
- **Plugin:** MCP tools in `integrations/memb-plugin/` — 9 tools: `add_memory`, `search_memories`, `get_memories`, `get_memory`, `update_memory`, `delete_memory`, `delete_all_memories`, `delete_entities`, `list_entities`

### Plugin & Skills System

- `integrations/memb-plugin/` provides integrations for Claude Code, Cursor, and Codex via MCP server connections and lifecycle hooks for automatic memory capture.
- `skills/` contains structured skill definitions for AI agents, split into two categories:
  - **Reference skills** (always-on SDK knowledge): `memb` (Python + TS SDKs, framework integrations), `memb-cli` (terminal workflows), `memb-vercel-ai-sdk` (Vercel AI provider).
  - **Pipeline skills** (run on demand): `memb-integrate` wires MemB into an existing repo via a TDD pipeline; `memb-test-integration` verifies what the integrator produced on the same branch (the two are loosely coupled via `.memb-integration/` artifacts); `memb-oss-to-platform` migrates an existing project from MemB OSS to the hosted Platform SDK (plan, then execute on approval).

### Adding a New Provider

To add a new LLM, embedding, vector store, or reranker provider:

1. Create `memb/<category>/<provider_name>.py`
2. Inherit from the abstract base class in `memb/<category>/base.py`
3. Add configuration to `memb/<category>/configs.py` (if the category uses one)
4. Register the provider in `memb/<category>/__init__.py`
...
| TypeScript SDK | `ts-sdk-ci.yml` | Push to main (on `memb-ts/`) | Prettier + build + jest on Node 20, 22 |
...
| OpenCode Plugin | `opencode-plugin-checks.yml` | Push to main (on `integrations/memb-plugin/.opencode-plugin/`), manual | Bun: tsc type-check + build + dist artifact check |
...
| Python SDK | `cd.yml` | `v*` | PyPI (`membai`) |
| TypeScript SDK | `ts-sdk-cd.yml` | `ts-v*` | npm (`membai`) |
| Python CLI | `cli-python-cd.yml` | `cli-v*` | PyPI (`memb-cli`) |
| Node CLI | `cli-node-cd.yml` | `cli-node-v*` | npm (`@memb/cli`) |
| Vercel AI SDK | `vercel-ai-cd.yml` | `vercel-ai-v*` | npm (`@memb/vercel-ai-provider`) |
| OpenClaw | `openclaw-cd.yml` | `openclaw-v*` | npm (`@memb/openclaw-memb`) |
| OpenCode Plugin | `opencode-plugin-cd.yml` | `opencode-v*` | npm (`@memb/opencode-plugin`) |
| Pi Agent Plugin | `pi-agent-plugin-cd.yml` | `pi-agent-v*` | npm (`@memb/pi-agent-plugin`) |
