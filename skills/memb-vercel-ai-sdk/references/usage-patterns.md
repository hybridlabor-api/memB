...
Memory retrieval happens before streaming begins. The conversation is stored to MemB as a fire-and-forget call (non-blocking).
...
const memb = createMemB(); // defaults to "openai"
const model = memb("gpt-5-mini", { user_id: "alice" });
```
...
const model = memb("claude-sonnet-4-20250514", { user_id: "alice" });
```

### Google

```typescript
const memb = createMemB({ provider: "google" });
const model = memb("gemini-2.0-flash", { user_id: "alice" });
```

### Groq

```typescript
const memb = createMemB({ provider: "groq" });
const model = memb("llama-3.3-70b-versatile", { user_id: "alice" });
```

### Cohere

```typescript
const memb = createMemB({ provider: "cohere" });
const model = memb("command-r-plus", { user_id: "alice" });
```
...
const memb = createMemB({
  ...
  membApiKey: "m0-xxx",   // MemB API key
});
```
...
2. processMemories(messagesPrompts, membConfig):
   a. addMemories(messagesPrompts, membConfig)
      --> fire-and-forget: .then().catch(), NO await
      --> POST /v3/memories/add/ with converted messages
   b. await getMemories(messagesPrompts, membConfig)
...
3. Create underlying LLM via MemBClassSelector.createProvider()
...
### Custom MemB API host

```typescript
const memb = createMemB({
  membConfig: {
    host: "https://my-memb-instance.example.com",
  },
});
```
...
### Default MemB config for all calls
...
// These calls inherit user_id and top_k from membConfig
const { text } = await generateText({
  model: memb("gpt-5-mini"),
  prompt: "Hello",
});
```

Per-call settings (passed as the second argument to `memb()`) are merged on top of `membConfig`, so you can override specific fields:

```typescript
// Override user_id for this specific call
const model = memb("gpt-5-mini", { user_id: "bob" });
```

The merge order is: `config.membConfig` (provider defaults) < `settings` (per-call overrides).
