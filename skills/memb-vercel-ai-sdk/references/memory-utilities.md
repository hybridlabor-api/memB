...
Complete reference for standalone utility functions exported from `@memb/vercel-ai-provider`. These functions give you manual control over memory retrieval and storage, independent of the wrapped model pattern.

Source: `integrations/vercel-ai-sdk/src/memb-utils.ts`

## `addMemories(messages, config?)`

Stores messages to MemB as new memories.
...
| `config` | `MemBConfigSettings` | Optional. Must include entity scope (`user_id`, etc.) and API key |

**Behavior:**
1. If `messages` is a string, wraps it as a single user message
2. Otherwise, converts `LanguageModelV2Prompt` to MemB format via `convertToMemBFormat` (handles multimodal content)
3. Calls `POST /v1/memories/` with the converted messages and config

**Returns:** The API response from MemB (memory operation result).
...
| `config` | `MemBConfigSettings` | Optional. Entity scope and API key |
...
```typescript
import { getMemories } from "@memb/vercel-ai-provider";
...
```typescript
import { searchMemories } from "@memb/vercel-ai-provider";
...
**Default host:** `https://api.memb.ai`

## Internal: `convertToMemBFormat(messages)`

Not exported. Used by `addMemories` to convert `LanguageModelV2Prompt` messages to MemB's format.
...
## `MemBConfigSettings` Fields Reference
...
| `host` | `string` | `https://api.memb.ai` | Custom API host |
