...
name: memb-integrate
description: >
  Integrate MemB into an existing repository using a goal-driven, TDD pipeline.
  Detects the repo's language automatically and asks the user to pick between
  MemB Platform (managed) and MemB Open Source (self-hosted). Writes failing
  tests before any implementation. Produces a local feature branch plus
  `.memb-integration/` artifacts consumed by the paired verification skill.
  TRIGGER when: user says "integrate memb", "add memb to this repo", "wire
  memb into <repo>", or asks how to add memory to an existing project.
  DO NOT TRIGGER when: the user wants general SDK usage (use skill:memb),
  CLI usage (use skill:memb-cli), or Vercel AI SDK (use skill:memb-vercel-ai-sdk).
  After success, invoke skill:memb-test-integration to verify in the same
...
metadata:
  ...
  memb_tested_versions: "membai (PyPI) >=2.0.0,<3.0.0; membai (npm) >=3.0.0,<4.0.0"
---

# memb-integrate

Wire MemB into an existing repo with a goal-driven, test-first pipeline.
Pairs with `memb-test-integration` for verification.
...
### Agent-ready docs
- Scope-tagged docs index: https://docs.memb.ai/llms.txt
- Full docs (single file, deep dives): https://docs.memb.ai/llms-full.txt
- OpenAPI spec (Platform REST, machine-readable): https://docs.memb.ai/openapi.json
- Hosted MCP server: https://mcp.memb.ai (requires Platform API key)
- Integrations index: https://docs.memb.ai/integrations

### Published MemB skills — delegate; do not reimplement
...
- SDK (Python + TS, Platform + OSS): https://raw.githubusercontent.com/membai/memb/main/skills/memb/SKILL.md
- CLI: https://raw.githubusercontent.com/membai/memb/main/skills/memb-cli/SKILL.md
- Vercel AI SDK: https://raw.githubusercontent.com/membai/memb/main/skills/memb-vercel-ai-sdk/SKILL.md
- Editor/MCP plugin glue (9 MCP tools): https://github.com/membai/memb/tree/main/integrations/memb-plugin

### SDK source (read when docs are ambiguous)
Public repo. Cross-check against the `memb_tested_versions` range in this
skill's frontmatter if the `main` branch has moved past a major.

- Repo root: https://github.com/membai/memb
- Python SDK: https://github.com/membai/memb/tree/main/memb
- TypeScript SDK: https://github.com/membai/memb/tree/main/memb-ts

### Quickstarts (for bootstrapping unfamiliar stacks)
- Platform: https://docs.memb.ai/platform/quickstart
- OSS Python: https://docs.memb.ai/open-source/python-quickstart
- OSS Node: https://docs.memb.ai/open-source/node-quickstart
...
4. **Minimal dependency surface.** Add `membai` (plus any deps the
...
6. **The null hypothesis wins.** If no additive, gated fit exists after
   ...
   than no PR.
7. **Backend only.** MemB integration lives in server-side code. API keys,
...
|---|---|---|
| `@ai-sdk/*` + `ai` in `package.json` | `skills/memb-vercel-ai-sdk` | Integration is via `createMemB` provider wrapper, not raw `MemoryClient`. |
| CLI-only repo (Typer, Commander, Click, Cobra) with no LLM call sites | `skills/memb-cli` | Call sites are command handlers, not model wrappers. Consider whether memb actually fits first. |
| Target is an MCP client / editor config (Claude Code, Cursor, Codex settings) | `integrations/memb-plugin` | Wire via MCP server URL + hooks; no SDK code usually needed. |
| Any other Python or TS repo with an LLM call site | `skills/memb` | Default SDK integration path. |
...
- Repo has a **backend**. Detected by: a `backend/` or `server/` or `api/`
  ...
  code 1 and a rationale. MemB is not installed client-side.
- The user has already decided MemB fits this repo. This skill does NOT
...
fit-surveying — the user already decided MemB fits. This is mechanics:
...
Produce `.memb-integration/repo-summary.md`:
...
`https://docs.memb.ai/llms.txt` for the Platform-first routing rules, then
...
> services throughout. I recommend **MemB Platform** (4-line integration).
...
Write `.memb-integration/goal.md` and **require user approval before step 6**.

Template:

    # MemB Integration Goal
...
- Check `package.json` / `pyproject.toml` / `requirements.txt` for
  conflicts (e.g., existing `membai` at a different version).

Then write `.memb-integration/plan.md`:

    # MemB Integration Plan
    ...
    **Sources consulted:** <minimum 2 URLs from "Canonical sources" above
    that informed this plan. At least one `docs.memb.ai` URL and one
    ...
    **E2E recipe:** <how the verification skill should drive the app
    ...
    entry point — in which case the E2E step will skip with a warning.>
        ...
        compose_services:    <optional: whitespace-separated service
                              ...
                              use label memb-e2e: "true" to mark them>
        write_call:          <command that triggers the MemB write path
                              exactly once; ≤ 60s runtime>
        write_async_wait_ms: <milliseconds to wait after write_call for
                              async memory flush; default 0>
        read_call:           <command that triggers the MemB read path,
...
- Platform method signatures: `https://docs.memb.ai/openapi.json`
  (request body schemas for `/v1/memories/` and `/v1/memories/search/`).
- OSS method signatures: the delegated skill named in `plan.md`
  (fetched from its raw URL) or `skills/memb/SKILL.md` as the default.
...
- `test_memb_write.<ext>` — asserts `add()` is called at the Write call
...
- **Inputs**: the repo, `goal.md`, `plan.md`, the two test files, and
  ...
  (pinned per `memb_tested_versions`), `https://docs.memb.ai/llms.txt`,
  and `https://docs.memb.ai/openapi.json`.
- **No access** to main agent's reasoning trace or scratchpad.
- **System prompt** (verbatim):

      You are implementing a MemB integration for an existing repo.
      ...
      - https://docs.memb.ai/llms.txt
      - https://docs.memb.ai/openapi.json  (Platform only)
      ...
      4. Gate every line of new MemB code behind the feature flag from
...
Create branch `memb-integrate/<short-goal-slug>` and commit in
**separate commits** so reviewers can cherry-pick:

1. `memb: add gated dependency` — just the `pyproject.toml` / `package.json`
   change.
2. `memb: add integration module` — the new file(s).
3. `memb: wire into <call site>` — the call-site edit(s), still gated.
4. `memb: add tests` — the new test files.

If `--no-heal` is set → print `Run /memb-test-integration to verify.`
and exit. Otherwise proceed to step 10.

### 10. Self-healing loop (default ON; disable with `--no-heal`)

Run `/memb-test-integration --ci` in a subprocess. If `scorecard.json`
...
2. **Spawn a remediation subagent**, fresh context. Inputs:
   ...
   System prompt (verbatim):
       ...
       You are fixing a failing MemB integration test.
...
3. **Apply the diff**; commit on the same branch with message
   `memb-heal: <category> attempt <N>`. Do NOT amend earlier commits
   (reviewers need the heal trail).

4. **Re-run `/memb-test-integration --ci`**. Outcomes:
   ...
   - A *different* check now failing → regression. Revert the heal
     ...
     `.memb-integration/heal-trace.md`, exit code 6.
...
6. **Post-loop summary** written to `.memb-integration/heal-trace.md`:
...
## Artifacts (all under `.memb-integration/`)
...
## Invocation
    ...
    /memb-integrate                            # interactive, heal ON
    /memb-integrate --no-heal                  # stop after commit; manual verify
    /memb-integrate --heal-max 5               # cap heal attempts per category (default 3)
    /memb-integrate --product platform         # skip the product ask
    /memb-integrate --product oss
    /memb-integrate --ci                       # non-interactive (for test harness)
...
- Surveying the repo for fit points. Humans decide where MemB helps before
  invoking this skill.
