...
Practical recipes for using the memb CLI in scripts, pipelines, and agent loops.
...
```bash
echo "I prefer dark mode" | memb add --user-id alice
```

### Pipe multi-line content

```bash
cat <<EOF | memb add --user-id alice
...
```bash
git log --oneline -5 | memb add --user-id ci-bot --metadata '{"source":"git"}'
```

### Search from pipe

```bash
echo "preferences" | memb search --user-id alice
```

### Update from pipe

```bash
echo "Updated: prefers dark mode AND high contrast" | memb update abc-123-def-456
```
...
Use `memb import` to bulk-load memories from a JSON file.
...
```bash
memb import data.json --user-id alice -o json
```
...
```bash
memb search "preferences" --user-id alice --agent
```
...
```bash
memb add "Uses Python 3.12" --user-id alice --json
```
...
```bash
memb search "test" --user-id alice --api-key invalid --agent
```
...
```bash
memb list --user-id alice --output json | jq '.[] | .memory'
```

### Get memory IDs

```bash
memb list --user-id alice -o json | jq '.[].id'
```

### Count memories

```bash
memb list --user-id alice -o json | jq 'length'
```

### Filter by category in jq

```bash
memb list --user-id alice -o json | jq '[.[] | select(.categories[]? == "preferences")]'
```

### Extract search scores

```bash
memb search "tools" --user-id alice -o json | jq '.[] | {memory, score}'
```
...
memb list --user-id alice -o json | jq -r '.[].id' | while read id; do
  memb delete "$id" --force
done
```
...
```bash
memb list --user-id alice -o json | jq -r '.[].memory' | while IFS= read -r mem; do
  memb add "$mem" --user-id bob
done
```
...
```bash
memb list --user-id alice -o json > alice_memories.json
```
...
page=1
while true; do
  result=$(memb list --user-id alice -o json --page "$page" --page-size 100)
...
```bash
memb add "Build #${BUILD_NUMBER} deployed ${APP_VERSION} to ${ENVIRONMENT} at $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  --agent-id "ci-bot" \
...
```bash
memb search "deployment to production" --agent-id ci-bot -o json -k 10
```

### Check CLI connectivity in CI

```bash
if memb status -o json | jq -e '.data.connected' > /dev/null 2>&1; then
  echo "memb is connected"
else
  echo "memb connection failed" >&2
  exit 1
fi
```
...
memb add "$test_summary" --agent-id ci-bot --metadata '{"type":"test-results"}' --categories "ci,testing"
```
...
**This means:**
- `memb add --user-id alice` in an interactive terminal will NOT hang waiting for input. It will print a usage error.
- `echo "text" | memb add --user-id alice` will read "text" from stdin.
- `memb add "explicit text" --user-id alice` will use the explicit text, even if stdin is piped.
...
memb status > /dev/null 2>&1

# Add with error check
if memb add "test memory" --user-id alice 2>/dev/null; then
...
result=$(memb add "new fact" --user-id alice --agent 2>/dev/null)
...
count=$(memb search "dark mode" --user-id alice --agent 2>/dev/null | jq '.count // 0')
if [ "$count" -eq 0 ]; then
  memb add "User prefers dark mode" --user-id alice
fi
```
...
memb add "background note" --user-id alice --output quiet 2>/dev/null
memb delete --all --user-id temp-user --force --output quiet 2>/dev/null
```
...
```bash
if ! memb search "query" --user-id alice -o json 2>/dev/null; then
...
Memories are processed asynchronously after `memb add`. If you need to search for a newly added memory immediately, add a short delay:

```bash
memb add "new preference" --user-id alice
sleep 3
memb search "new preference" --user-id alice
```
...
result=$(memb add "new preference" --user-id alice --agent 2>/dev/null)
event_id=$(echo "$result" | jq -r '.data[0].event_id // empty')

if [ -n "$event_id" ]; then
  # Poll until processing completes
  while true; do
    status=$(memb event status "$event_id" --agent 2>/dev/null | jq -r '.data.status')
...
case "$ACTION" in
  recall)
    memb search "$*" --user-id "$USER_ID" --agent 2>/dev/null
    ;;
  remember)
    memb add "$*" --user-id "$USER_ID" --agent 2>/dev/null
    ;;
  forget)
    memb delete --all --user-id "$USER_ID" --force --agent 2>/dev/null
    ;;
  history)
    memb list --user-id "$USER_ID" --agent 2>/dev/null
    ;;
  *)
