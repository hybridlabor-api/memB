---
name: memb-test-integration
description: >
  Verify a MemB integration produced by /memb-integrate. Runs in the same
  ...
  TRIGGER when: user has just run /memb-integrate and says "verify",
  "test the integration", "run /memb-test-integration", or when a
  .memb-integration/ directory exists and tests have not been run yet
  on the current branch.
  DO NOT TRIGGER when: the user wants to run general project tests
  (defer to the repo's native test command), or when no prior /memb-integrate
  run exists in the current branch (ask them to run /memb-integrate first).
...
# memb-test-integration

Verifies what `/memb-integrate` produced. Runs in the same workspace,
...
- OpenAPI (Platform REST): https://docs.memb.ai/openapi.json
- Published SDK skill (canonical call patterns): https://raw.githubusercontent.com/membai/memb/main/skills/memb/SKILL.md
- Vercel AI SDK skill (if the target repo uses `@ai-sdk/*`): https://raw.githubusercontent.com/membai/memb/main/skills/memb-vercel-ai-sdk/SKILL.md
- SDK source (cross-check version against frontmatter `memb_tested_versions`):
  - Repo root: https://github.com/membai/memb
  - Python: https://github.com/membai/memb/tree/main/memb
  - TypeScript: https://github.com/membai/memb/tree/main/memb-ts

Read the `Delegated skill:` field in `.memb-integration/plan.md` — if it
...
feature-flagged** (see `/memb-integrate` "Integration principles").
Specifically:
...
- `.memb-integration/product.json`, `goal.md`, and `plan.md` are readable
  and internally consistent (JSON parses, docs non-empty).
- Current branch name begins with `memb-integrate/` (set by the companion
...
- `product.json` → which language, which product (Platform vs OSS), which
  memb version, `write_site`, `read_site`.
...
| Language | Command |
|---|---|
| Python | `pip install -e .` if editable, else `pip install -r requirements.txt`. Then `pip install membai` if not already present at the pinned version. |
...
- **Import check**: does the write-site file import the expected MemB
  surface? Authoritative list comes from `## Identify the User's Setup`
  in `https://docs.memb.ai/llms.txt`:
  - Platform Python → `from memb import MemoryClient`
  - Platform TS → `import MemoryClient from "membai"`
  - OSS Python → `from memb import Memory`
  - OSS TS → `import { Memory } from "membai/oss"`
  ...
  with line number.
- **Version check**: installed `membai` version falls in the range from
  this skill's `memb_tested_versions`. Out of range → warn but continue.
...
- **Eager-init check**: grep the `write_site` and `read_site` files (paths
  ...
  the lazy-init guidance from `/memb-integrate` step 8 constraint #7.
...
(excluding the new `test_memb_*` files). **Must be 100% green.** Any
...
Capture output to `.memb-integration/test-stdout-flag-off.log` and
`.memb-integration/test-stdout-flag-on.log`. Scorecard reports pass/fail
per pass.
...
**Platform (TS):** same shape with `MemoryClient` from `"membai"`.
...
`memb-test-integration-` so a failed cleanup doesn't pollute the user's
real data. A background tidy step deletes any prefix-matching entries
older than 24 hours on the next run.

Capture output to `.memb-integration/smoke-stdout.log`.
...
`/memb-integrate` step 5). If absent → status `skipped` (not `fail`),
...
- `write_call` — triggers the MemB write path exactly once. Output is
...
- `read_call` — triggers the MemB read path. Typically a fresh session
...
10. Cleanup (always, even on failure): SIGTERM the app, SIGKILL after
    ...
    memories matching `memb-test-integration-*` on Platform scenarios.
...
Write `.memb-integration/scorecard.md` and `.memb-integration/scorecard.json`:

    {
      "timestamp": "2026-04-20T14:03:11Z",
      "branch": "memb-integrate/remember-user-preferences",
      "product": "platform",
      "language": "python",
      "memb_version": "2.0.0",
      "non_invasive": true,
      ...
      "friction": {
        ...
        "warnings": ["membai 2.0.0 pinned; consider 2.0.1 for fix X"]
      },
      "overall": "pass"
    }
...
- Friction summary.
- Verbatim warnings from memb SDK (if any — e.g., deprecated field usage).
...
- **Do not commit the scorecard files.** They live in `.memb-integration/`,
...
## Invocation
    ...
    /memb-test-integration                       # interactive, all steps
    /memb-test-integration --ci                  # non-interactive
    /memb-test-integration --skip-smoke          # no API calls, no E2E
    /memb-test-integration --skip-e2e            # unit + smoke only (faster CI)
    /memb-test-integration --only-smoke          # just smoke
    /memb-test-integration --only-e2e            # just E2E (assumes deps installed)
...
|---|---|
| 0 | All checks passed. |
| 1 | Precondition failed (no `.memb-integration/`, wrong branch, dirty tree). |
...
- **Modifying source files.** The skill is read-only against the repo.
  If verification exposes a bug, re-run `/memb-integrate` on the same
...
- **Self-healing.** This skill never modifies source files. The paired
  `/memb-integrate` skill in its default `--heal` mode consumes the
