#!/usr/bin/env python3
"""
MemB Documentation Search Agent (Mintlify-based)
On-demand search tool for querying MemB documentation without storing content locally.

This tool leverages Mintlify's documentation structure to perform just-in-time
retrieval of technical information from docs.memb.ai.

Usage:
    python memb_doc_search.py --query "how to add graph memory"
    python memb_doc_search.py --query "filter syntax for categories"
    python memb_doc_search.py --page "/platform/features/graph-memory"
    python memb_doc_search.py --index
    python memb_doc_search.py --query "webhook events" --section platform
...
Purpose:
    ...
    - Search across the full MemB documentation site
"""
...
def fetch_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "MemBDocSearchAgent/1.0"})
    try:
...
def search_docs(query: str, section: str | None = None) -> dict:
    """
    Search MemB documentation using Mintlify's search API.
...
def main():
    parser = argparse.ArgumentParser(description="Search MemB documentation on demand")
