...
- A MemB Platform API key ([Get one here](https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=skill-memb-quickstart))
...
# Add memory
curl -X POST https://api.memb.ai/v1/memories/ \
...
# Search memories
curl -X POST https://api.memb.ai/v2/memories/search/ \
  -H "Authorization: Token $MEMB_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
