# MemB Use Cases & Examples

Real-world implementation patterns for MemB Platform. Each use case includes complete, runnable code in both Python and TypeScript.
...
A fitness coach that remembers goals, preferences, and progress across sessions. MemB persists context across app restarts — no session state needed.
...
def chat(user_input: str, user_id: str) -> str:
    ...
    memories = memb.search(user_input, user_id=user_id)
...
async function chat(userInput: string, userId: string): Promise<string> {
    // 1. Retrieve relevant memories
    const memories = await memb.search(userInput, { filters: { user_id: userId } });
...
def consult(user_id: str, question: str) -> str:
    ...
    memories = memb.search(question, user_id=user_id, top_k=5, threshold=0.7)
    context = "\n".join([f"- {m['memory']}" for m in memories.get("results", [])])
...
async function consult(userId: string, question: string): Promise<string> {
    const memories = await memb.search(question, {
...
async function draftContent(userId: string, topic: string): Promise<string> {
    const prefs = await memb.search('writing style preferences', {
...
# One-time setup: configure MemB to infer from queries
memb.project.update(
...
def personalized_search(user_id: str, query: str, search_results: list) -> str:
    ...
    memories = memb.search(query, user_id=user_id, top_k=5)
    user_context = "\n".join([f"- {m['memory']}" for m in memories.get("results", [])])
...
# Over time, MemB learns: "user prefers vegetarian, lives in Austin"
...
async function personalizedSearch(userId: string, query: string, searchResults: string[]): Promise<string> {
    const memories = await memb.search(query, { filters: { user_id: userId }, topK: 5 });
    const context = memories.results?.map((m: any) => `- ${m.memory}`).join('\n') || '';
    ...
    await memb.add([{ role: 'user', content: query }], { userId: userId });
    return reply;
}
```
...
Control what MemB extracts from conversations:
...
For 30+ cookbooks with complete working code: [docs.memb.ai/cookbooks](https://docs.memb.ai/cookbooks)
