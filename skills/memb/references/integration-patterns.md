# MemB Integration Patterns

Working code examples for integrating MemB Platform with popular AI frameworks.
All examples use `MemoryClient` (Platform API key).

Code examples are sourced from official MemB integration docs at docs.memb.ai, simplified for quick reference.
...
3. **Store** -- save the interaction back to MemB for future use

---

## LangChain

Source: [docs.memb.ai/integrations/langchain](https://docs.memb.ai/integrations/langchain)
...
def retrieve_context(query: str, user_id: str):
    """Retrieve relevant memories from MemB"""
    memories = memb.search(query, user_id=user_id)
...
Source: [docs.memb.ai/integrations/crewai](https://docs.memb.ai/integrations/crewai)

CrewAI has native MemB integration via `memory_config`:
...
> **Dedicated skill available.** For comprehensive Vercel AI SDK documentation, see the [memb-vercel-ai-sdk skill](../memb-vercel-ai-sdk/SKILL.md) ([GitHub](https://github.com/membai/memb/tree/main/skills/memb-vercel-ai-sdk)).

Install: `npm install @memb/vercel-ai-provider`
...
const { text } = await generateText({
    model: memb("gpt-5-mini", { user_id: "borat" }),
...
Source: [docs.memb.ai/integrations/openai-agents-sdk](https://docs.memb.ai/integrations/openai-agents-sdk)
...
def search_memory(query: str, user_id: str) -> str:
    ...
    memories = memb.search(query, user_id=user_id, top_k=3)
...
def save_memory(content: str, user_id: str) -> str:
    ...
    memb.add([{"role": "user", "content": content}], user_id=user_id)
...
Source: [docs.memb.ai/integrations/pipecat](https://docs.memb.ai/integrations/pipecat)
...
Source: [docs.memb.ai/integrations/langgraph](https://docs.memb.ai/integrations/langgraph)
...
def chatbot(state: State):
    ...
    user_id = state["memb_user_id"]

    # Retrieve relevant memories
    memories = memb.search(messages[-1].content, user_id=user_id)
...
Source: [docs.memb.ai/integrations/llama-index](https://docs.memb.ai/integrations/llama-index)

Install: `pip install llama-index-core llama-index-memory-memb`

LlamaIndex has native MemB support via `MemBMemory`. Works with ReAct and FunctionCalling agents.

```python
from llama_index.memory.memb import MemBMemory

context = {"user_id": "alice", "agent_id": "llama_agent_1"}
memory = MemBMemory.from_client(
    context=context,
...
# Agent retrieves the vegetarian preference from MemB
```
...
Source: [docs.memb.ai/integrations/autogen](https://docs.memb.ai/integrations/autogen)

Install: `pip install autogen membai`
...
Beyond the examples above, MemB integrates with:

| Framework | Type | Install |
|-----------|------|---------|
| [Mastra](https://docs.memb.ai/integrations/mastra) | TS agent framework | `npm install @mastra/memb` |
| [ElevenLabs](https://docs.memb.ai/integrations/elevenlabs) | Voice AI | `pip install elevenlabs membai` |
| [LiveKit](https://docs.memb.ai/integrations/livekit) | Real-time voice/video | `pip install livekit-agents membai` |
| [Camel AI](https://docs.memb.ai/integrations/camel-ai) | Multi-agent framework | `pip install camel-ai[all] membai` |
| [AWS Bedrock](https://docs.memb.ai/integrations/aws-bedrock) | Cloud LLM provider | `pip install boto3 membai` |
| [Dify](https://docs.memb.ai/integrations/dify) | Low-code AI platform | Plugin-based |
| [Google AI ADK](https://docs.memb.ai/integrations/google-ai-adk) | Google agent framework | `pip install google-adk membai` |
