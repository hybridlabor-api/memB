...
from memb.memory.main import _entity_collection_name
from memb.memory.utils import normalize_facts
...
def test_entity_collection_name_keeps_underscore_for_other_stores():
    assert _entity_collection_name("qdrant", "memb") == "memb_entities"
...
@patch('memb.utils.factory.EmbedderFactory.create')
@patch('memb.utils.factory.VectorStoreFactory.create')
@patch('memb.utils.factory.LlmFactory.create')
@patch('memb.memory.storage.SQLiteManager')
...
def test_search_handles_incomplete_payloads(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    ...
    from memb.memory.main import Memory as MemoryClass
    config = MemoryConfig()
...
@patch('memb.memory.main.extract_entities', return_value=[])
@patch('memb.utils.factory.EmbedderFactory.create')
@patch('memb.utils.factory.VectorStoreFactory.create')
@patch('memb.utils.factory.LlmFactory.create')
@patch('memb.memory.storage.SQLiteManager')
...
async def test_async_read_apis_surface_attributed_to(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    ...
    from memb.memory.main import AsyncMemory
...
@patch('memb.utils.factory.VectorStoreFactory.create')
@patch('memb.utils.factory.LlmFactory.create')
@patch('memb.memory.main.SQLiteManager')
...
class TestHybridSearchWarning:
    ...
    @patch("memb.memory.telemetry.capture_event")
    @patch("memb.memory.main.SQLiteManager")
    @patch("memb.utils.factory.LlmFactory.create")
    @patch("memb.utils.factory.EmbedderFactory.create")
    @patch("memb.utils.factory.VectorStoreFactory.create")
    def test_warning_for_store_without_keyword_search(
        ...
        with caplog.at_level(logging.WARNING, logger="memb.memory.main"):
            Memory(config)
...
@pytest.mark.asyncio
@patch("memb.memory.main.VectorStoreFactory")
@patch("memb.memory.main.EmbedderFactory")
@patch("memb.memory.main.LlmFactory")
