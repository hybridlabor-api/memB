...
class TestSetupConfigIdempotent:
    def test_creates_config_when_missing(self, tmp_memb_dir):
        import memb.memory.setup as setup_module

        setup_module.setup_config()
        config = json.loads((tmp_memb_dir / "config.json").read_text())
        assert "user_id" in config and config["user_id"]

    def test_backfills_user_id_when_only_telemetry_present(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
    ...
    def test_does_not_overwrite_existing_user_id(self, tmp_memb_dir):
        import memb.memory.setup as setup_module

        _write_config(tmp_memb_dir, {"user_id": "existing-uuid"})
        setup_module.setup_config()
        config = json.loads((tmp_memb_dir / "config.json").read_text())
        assert config["user_id"] == "existing-uuid"

    def test_handles_malformed_json(self, tmp_memb_dir):
        import memb.memory.setup as setup_module

        (tmp_memb_dir / "config.json").write_text("{not json")
...
class TestReadAnonIds:
    def test_returns_oss_only(self, tmp_memb_dir):
        import memb.memory.setup as setup_module

        _write_config(tmp_memb_dir, {"user_id": "oss-uuid"})
        anon = setup_module.read_anon_ids()
        assert anon == {"oss": "oss-uuid", "cli": None, "aliased_pairs": []}

    def test_returns_cli_only(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
    ...
    def test_returns_both(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
    ...
    def test_no_config_returns_all_none(self, tmp_memb_dir):
        import memb.memory.setup as setup_module

        anon = setup_module.read_anon_ids()
        assert anon == {"oss": None, "cli": None, "aliased_pairs": []}

    def test_malformed_json_does_not_raise(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
...
class TestMarkAliased:
    def test_writes_aliased_pair_preserving_other_fields(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
    ...
    def test_creates_telemetry_section_when_missing(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
    ...
    def test_tracks_each_pair_independently(self, tmp_memb_dir):
        import memb.memory.setup as setup_module
...
class TestEndToEndIdempotency:
    ...
    def test_second_call_is_noop_after_pair_marker_persisted(self, tmp_memb_dir):
