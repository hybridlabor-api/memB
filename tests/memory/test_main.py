...
from memb.memory.main import AsyncMemory, Memory
...
def _setup_mocks(mocker):
    ...
    mocker.patch("memb.utils.factory.EmbedderFactory.create", mock_embedder)

    mock_vector_store = mocker.MagicMock()
    mock_vector_store.return_value.search.return_value = []
    mocker.patch(
        "memb.utils.factory.VectorStoreFactory.create", side_effect=[mock_vector_store.return_value, mocker.MagicMock()]
    )

    mock_llm = mocker.MagicMock()
    mocker.patch("memb.utils.factory.LlmFactory.create", mock_llm)

    mocker.patch("memb.memory.storage.SQLiteManager", mocker.MagicMock())
...
class TestAddToVectorStoreErrors:
    ...
    def test_empty_llm_response_fact_extraction(self, mocker, mock_memory, caplog):
        ...
        mocker.patch("memb.memory.main.capture_event")
...
class TestAsyncAddToVectorStoreErrors:
    ...
    async def test_async_empty_llm_response_fact_extraction(self, mock_async_memory, caplog, mocker):
        ...
        mocker.patch("memb.utils.factory.EmbedderFactory.create", return_value=MagicMock())
    ...
    async def test_async_empty_llm_response_memory_actions(self, mock_async_memory, caplog, mocker):
        ...
        mocker.patch("memb.memory.main.capture_event", mock_capture_event)
...
def _build_memory_instance(mocker, memory_cls):
    _setup_mocks(mocker)
    mocker.patch("memb.memory.main.SQLiteManager", mocker.MagicMock())
    mocker.patch("memb.memory.main.MEM0_TELEMETRY", False)
    memory = memory_cls()
    memory.config = mocker.MagicMock()
    memory.config.custom_instructions = None
    memory.config.custom_update_memory_prompt = None
...
class TestEntityBoostParallelism:
    ...
    def test_sync_boosts_preserve_scoring(self, mock_memory):
        from memb.utils.scoring import ENTITY_BOOST_WEIGHT
...
class TestAddPipelineEntityEmbeddingCountGuard:
    ...
    def test_sync_short_entity_embeddings_still_link_valid_entity(self, mock_memory, mocker, caplog):
        ...
        mocker.patch(
            "memb.memory.main.extract_entities_batch",
            return_value=[
                [("person", "Alice"), ("person", "Bob")],
