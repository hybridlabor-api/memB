...
from memb.llms.azure_openai import AzureOpenAILLM
...
def mock_openai_client():
    with patch("memb.llms.azure_openai.AzureOpenAI") as mock_openai:
        mock_client = Mock()
...
def test_is_reasoning_model_override_forces_reasoning_path(mock_openai_client):
    ...
    Regression test for https://github.com/membai/memb/issues/5296 — the
...
def test_generate_with_http_proxies(default_headers):
    ...
    with (
        patch("memb.llms.azure_openai.AzureOpenAI") as mock_azure_openai,
        ...
        config = AzureOpenAIConfig(
            ...
            top_p=TOP_P,
            api_key="test",
            http_client_proxies="http://testproxy.memb.net:8000",
        ...
        mock_http_client.assert_called_once_with(proxy="http://testproxy.memb.net:8000")
...
def test_init_with_api_key(monkeypatch):
    ...
    with patch("memb.llms.azure_openai.AzureOpenAI") as mock_azure_openai:
        llm = AzureOpenAILLM(config)
        mock_azure_openai.assert_called_once_with(
...
def test_init_with_default_azure_credential(monkeypatch):
    ...
    with (
        patch("memb.llms.azure_openai.DefaultAzureCredential") as mock_cred,
        patch("memb.llms.azure_openai.get_bearer_token_provider") as mock_token_provider,
        patch("memb.llms.azure_openai.AzureOpenAI") as mock_azure_openai,
    ):
