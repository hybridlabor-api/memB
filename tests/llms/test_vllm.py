...
from memb import AsyncMemory, Memory
from memb.configs.llms.base import BaseLlmConfig
from memb.llms.vllm import VllmLLM


@pytest.fixture
def mock_vllm_client():
    with patch("memb.llms.vllm.OpenAI") as mock_openai:
        mock_client = Mock()
...
def create_mocked_memory():
    ...
    with patch('memb.utils.factory.LlmFactory.create') as mock_llm_factory, \
         patch('memb.utils.factory.EmbedderFactory.create') as mock_embedder_factory, \
         patch('memb.utils.factory.VectorStoreFactory.create') as mock_vector_factory, \
         patch('memb.memory.storage.SQLiteManager') as mock_sqlite:
...
async def test_async_thinking_tags_async():
    ...
    with patch('memb.memory.main.asyncio.to_thread', side_effect=mock_to_thread):
