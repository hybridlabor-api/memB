...
from memb.embeddings.vertexai import VertexAIEmbedding


@pytest.fixture
def mock_text_embedding_model():
    with patch("memb.embeddings.vertexai.TextEmbeddingModel") as mock_model:
        mock_instance = Mock()
...
@pytest.fixture
def mock_os_environ():
    with patch("memb.embeddings.vertexai.os.environ", {}) as mock_environ:
        yield mock_environ


@pytest.fixture
def mock_config():
    with patch("memb.configs.embeddings.base.BaseEmbedderConfig") as mock_config:
...
def mock_text_embedding_input():
    with patch("memb.embeddings.vertexai.TextEmbeddingInput") as mock_input:
        yield mock_input


@patch("memb.embeddings.vertexai.TextEmbeddingModel")
...
@patch("memb.embeddings.vertexai.os")
...
@patch("memb.embeddings.vertexai.TextEmbeddingInput")
@patch("memb.embeddings.vertexai.TextEmbeddingModel")
