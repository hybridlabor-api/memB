...
from memb.embeddings.azure_openai import AzureOpenAIEmbedding


@pytest.fixture
def mock_openai_client():
    with patch("memb.embeddings.azure_openai.AzureOpenAI") as mock_openai:
        mock_client = Mock()
...
def test_init_with_api_key(monkeypatch, base_embedder_config):
    ...
    with (
        patch("memb.embeddings.azure_openai.AzureOpenAI") as mock_azure_openai,
        patch("memb.embeddings.azure_openai.DefaultAzureCredential") as mock_cred,
        patch("memb.embeddings.azure_openai.get_bearer_token_provider") as mock_token_provider,
    ):
...
def test_init_with_env_vars(monkeypatch, base_embedder_config):
    ...
    with patch("memb.embeddings.azure_openai.AzureOpenAI") as mock_azure_openai:
        AzureOpenAIEmbedding(base_embedder_config)
        mock_azure_openai.assert_called_once_with(
