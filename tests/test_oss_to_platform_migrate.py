...
def write_config(memb_dir: Path, data: dict[str, Any]) -> None:
    memb_dir.mkdir(parents=True, exist_ok=True)
    (memb_dir / "config.json").write_text(json.dumps(data), encoding="utf-8")


def read_config(memb_dir: Path) -> dict[str, Any]:
    return json.loads((memb_dir / "config.json").read_text(encoding="utf-8"))
...
def run_migration_script(
    ...
    *args: str,
    ...
    memb_dir = tmp_path / "memb"
    if config is not None:
        write_config(memb_dir, config)
    if raw_config is not None:
        memb_dir.mkdir(parents=True, exist_ok=True)
        (memb_dir / "config.json").write_text(raw_config, encoding="utf-8")
...
def run_export_script(
    ...
    *args: str,
    ...
    return result, memb_dir, output_path
...
def test_existing_api_key_authenticates_and_stitches_ids(tmp_path: Path) -> None:
    config = {
        "user_id": "oss-123",
        "platform": {"api_key": "stored-key", "base_url": "https://api.memb.ai"},
        "telemetry": {"anonymous_id": "cli-456"},
    }

    with MigrationHTTPServer(ping_emails={"stored-key": "bob@example.com"}) as server:
        result, memb_dir = run_migration_script(tmp_path, server, "--yes", config=config)
...
def test_email_code_authenticates_without_persisting_credentials(tmp_path: Path) -> None:
    with MigrationHTTPServer(ping_emails={"verified-key": "alice@example.com"}) as server:
        result, memb_dir = run_migration_script(
            tmp_path,
            server,
            "--email",
            "Alice@Example.COM",
...
def test_invalid_stored_key_falls_back_to_email_code(tmp_path: Path) -> None:
    config = {
        ...
        "platform": {"api_key": "bad-key", "base_url": "https://api.memb.ai"},
    }
    ...
    assert result.returncode == 0, result.stderr
    assert "Stored MemB Platform API key is invalid or expired" in result.stdout
...
def test_missing_python3_prints_clear_shell_error(tmp_path: Path) -> None:
    ...
    assert result.returncode == 1
    assert "python3 is required to run the MemB migration" in result.stderr
...
def test_export_qdrant_memories_to_json_without_vectors_or_api_key(tmp_path: Path) -> None:
    ...
    with MigrationHTTPServer(qdrant_pages=pages) as server:
        result, _memb_dir, output_path = run_export_script(
            tmp_path,
            server,
            "--user-id",
            "alice",
    ...
    assert artifact["kind"] == "memb_oss_qdrant_export"
...
def test_export_requires_scope_or_all(tmp_path: Path) -> None:
    with MigrationHTTPServer() as server:
        result, _memb_dir, output_path = run_export_script(tmp_path, server)
...
def test_export_all_uses_no_qdrant_filter(tmp_path: Path) -> None:
    with MigrationHTTPServer(qdrant_pages=[{"points": [], "next_page_offset": None}]) as server:
        result, _memb_dir, output_path = run_export_script(tmp_path, server, "--all")
...
def test_import_platform_memories_from_export_json(tmp_path: Path) -> None:
    ...
    input_path.write_text(
        json.dumps(
            {
                "source": {"sdk": "python", "vector_store": "qdrant", "collection": "memb_test"},
                "records": [
                    {
                        "id": "local-1",
    ...
    with MigrationHTTPServer(ping_emails={"import-key": "alice@example.com"}) as server:
        result, _memb_dir = run_import_script(tmp_path, server, input_path)
    ...
    assert body["metadata"]["memb_migration_source"] == "python_oss_qdrant"
    assert body["metadata"]["memb_migration_collection"] == "memb_test"
    assert body["metadata"]["memb_migration_local_id"] == "local-1"
    assert body["metadata"]["memb_migration_local_hash"] == "hash-1"
...
def test_import_skips_existing_identical_memory(tmp_path: Path) -> None:
    ...
    import_key = sha256("python:qdrant:memb_test:local-1".encode("utf-8")).hexdigest()
    ...
    existing = [
        {
            ...
            "user_id": "alice",
            "metadata": {
                "memb_migration_import_key": import_key,
                "memb_migration_local_hash": "hash-1",
            },
        }
    ]
    ...
    existing = [
        {
            ...
            "metadata": {
                "memb_migration_import_key": import_key,
                "memb_migration_local_hash": "hash-old",
            },
        }
    ]
...
def test_full_flow_auth_export_and_imports_memories(tmp_path: Path) -> None:
    ...
    with MigrationHTTPServer(ping_emails={"verified-key": "alice@example.com"}, qdrant_pages=qdrant_pages) as server:
        result, _memb_dir, output_path = run_full_script(
            tmp_path,
            server,
            "--email",
            "alice@example.com",
    ...
    assert result.returncode == 0, result.stderr
    assert "Phase 1/3: Authenticate with MemB Platform" in result.stdout
    assert "Phase 2/3: Export Python OSS memories from hosted Qdrant" in result.stdout
    assert "Phase 3/3: Import memories into MemB Platform" in result.stdout
    assert "Imported: 1" in result.stdout
