...
def _mock_memory():
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "fake-key", "ADMIN_API_KEY": ""}):
        with patch("memb.Memory.from_config", return_value=mock_instance):
            yield mock_instance
...
class TestWriteHandlerErrorMapping:
    """ValueError("... not found") -> 404, other ValueError / MemBValidationError
    ...
    def test_add_validation_error_returns_400(self, client, mock_memory):
        mock_memory.add.side_effect = MemBValidationError(
