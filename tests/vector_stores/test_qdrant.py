...
from memb.vector_stores.qdrant import Qdrant
...
class TestQdrant(unittest.TestCase):
    ...
    def test_local_path_on_disk_false_preserves_existing_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("memb.vector_stores.qdrant.QdrantClient", return_value=mock_client):
                Qdrant(
                    collection_name="c",
                    embedding_model_dims=128,
                    path=tmp,
...
class TestQdrantDatetimeRangeFilters(unittest.TestCase):
    ...
    def test_missing_fastembed_warns_with_extras_install_hint(self):
        """When fastembed is missing, the BM25 warning must point at the memb optional group
        that actually provides it (`membai[extras]`), not the bare `fastembed` package -
        ...
        with patch.dict("sys.modules", {"fastembed": None}):
            with self.assertLogs("memb.vector_stores.qdrant", level="WARNING") as cm:
        ...
        joined = "\n".join(cm.output)
        self.assertIn("membai[extras]", joined)  # points at the right install
