...
from memb.vector_stores.opensearch import OpenSearchDB
...
class TestOpenSearchDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch("memb.vector_stores.opensearch.OpenSearch", return_value=self.client_mock)
    ...
    def test_auto_refresh_enabled(self):
        ...
        with patch("memb.vector_stores.opensearch.OpenSearch", return_value=self.client_mock):
    ...
    @patch("memb.vector_stores.opensearch.logger")
    ...
    def test_init_with_http_auth(self):
        ...
        with patch("memb.vector_stores.opensearch.OpenSearch") as mock_opensearch:
            OpenSearchDB(
                host="localhost",
                port=9200,
                collection_name="test_collection",
...
def test_safe_deepcopy_config_handles_opensearch_auth(
    ...
    from memb.memory.main import _safe_deepcopy_config
...
def test_memory_initialization_opensearch_aws_auth(
    ...
    config.vector_store.config = MockOpenSearchConfig(collection_name="memb_test", include_auth=True)
