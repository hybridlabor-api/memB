...
def _make_redis_db():
    ...
    to avoid the redis module name collision with memb.vector_stores.redis."""
    from memb.vector_stores.redis import RedisDB

    db = RedisDB.__new__(RedisDB)
    mock_index = MagicMock()
    db.index = mock_index
    db.schema = {"index": {"prefix": "memb:test"}}
...
def test_create_col_keeps_distinct_dims_across_instances():
    ...
    global)."""
    import memb.vector_stores.redis as redis_module
...
def test_init_keeps_distinct_dims_from_module_global():
    ...
    with (
        patch("memb.vector_stores.redis.redis.Redis.from_url", return_value=MagicMock()),
