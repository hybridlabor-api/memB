...
from memb.vector_stores.turbopuffer import OutputData, TurbopufferDB
...
@pytest.fixture
def mock_client():
    with patch("memb.vector_stores.turbopuffer.TurbopufferClient") as MockClient:
...
class TestConfig:
    def test_config_valid(self):
        with patch.dict("os.environ", {"TURBOPUFFER_API_KEY": "key"}):
            from memb.configs.vector_stores.turbopuffer import TurbopufferConfig

            config = TurbopufferConfig()
            assert config.collection_name == "memb"
...
class TestFactoryRegistration:
    def test_vector_store_factory_has_turbopuffer(self):
        ...
        assert VectorStoreFactory.provider_to_class["turbopuffer"] == "memb.vector_stores.turbopuffer.TurbopufferDB"
