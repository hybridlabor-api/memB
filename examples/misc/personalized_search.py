"""
Personalized Search Agent with MemB + Tavily
Uses LangChain agent pattern with Tavily tools for personalized search based on user memories stored in MemB.
"""
...
def setup_user_history(user_id):
    ...
    for conversation in conversations:
        memb_client.add(conversation, user_id=user_id)


def get_user_context(user_id, query):
    """Retrieve relevant user memories from MemB"""
    try:
        ...
        user_memories = memb_client.search(
            query=query,
            version="v2",
            filters=filters
        )
...
def conduct_personalized_search(user_id, query):
    """
    Personalized search workflow using LangChain agent + Tavily + MemB
    ...
    try:
        ...
        # Store this search interaction in MemB for user preferences
...
def store_search_interaction(user_id, original_query, agent_response):
    """Store search interaction in MemB for future personalization"""
    try:
        interaction = [
        ...
        memb_client.add(messages=interaction, user_id=user_id)
