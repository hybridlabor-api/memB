...
- MemB for memory orchestration and lifecycle management
...
- AWS Blog: https://aws.amazon.com/blogs/database/build-persistent-memory-for-agentic-ai-applications-with-memb-open-source-amazon-elasticache-for-valkey-and-amazon-neptune-analytics/
- MemB Blog: https://memb.ai/blog/build-persistent-memory-for-agentic-ai-applications-with-memb-open-source-amazon-elasticache-for-valkey-and-amazon-neptune-analytics
...
Installation:
pip install strands-agents strands-agents-tools membai streamlit

Usage:
streamlit run agent1.py

Example queries:
1. "What is the URL for the project memb and its most important metrics?"
2. "Find the top contributors for MemB and store this information in a graph"
...
from memb.memory.main import Memory
...
def store_memory_tool(information: str, user_id: str = "user", category: str = "conversation") -> str:
    ...
    """
    try:
        # Create a simple message format for memb vector storage
...
def store_memory(messages, user_id="alice", category="conversation"):
    """
    Store the conversation thread in memb memory.
