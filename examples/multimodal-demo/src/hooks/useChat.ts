...
import { MemoryClient, Memory as MemBMemory } from 'membai';
...
export const useChat = ({ user, membApiKey, openaiApiKey }: UseChatProps): UseChatReturn => {
  ...
  const updateMemories = async (messages: PromptMessage[]) => {
    const memoryClient = new MemoryClient({ apiKey: membApiKey || '' });
    try {
      ...
      const newMemories = response.map((memory: MemBMemory) => ({
        id: memory.id || '',
      ...
      let relevantMemories = '';
        try {
          ...
          relevantMemories = searchResponse
            .map((memory: MemBMemory) => `Previous context: ${memory.memory}`)
            .join('\n');
        } catch (error) {
