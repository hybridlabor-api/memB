...
async function init() {
  ...
  await loadConfig();
  // Initialize MemBAI and load memories
  await initializeMemBAI();
  await fetchMemories();
}

// Initialize MemBAI with API key from storage
async function initializeMemBAI() {
  try {
    const response = await chrome.runtime.sendMessage({ action: "getConfig" });
    const membApiKey = response.config.membApiKey;

    if (!membApiKey) {
      showMemoriesError("Please configure your MemB API key in the popup");
      return false;
    }
    ...
    showMemoriesError("Failed to initialize MemBAI");
    return false;
  }
}
...
async function fetchMemories() {
  try {
    ...
    const memories = await membclient.getAll({
...
async function saveMemory() {
  ...
  try {
    ...
    await membclient.update(currentMemory.id, updatedMemory);
...
async function deleteMemory(memoryId) {
  ...
  try {
    ...
    await membclient.delete(memoryId);
...
async function addMemory() {
  ...
  try {
    ...
    const result = await membclient.add(
      [
        {
          role: "user",
          content: content,
