...
async function init() {
  try {
    ...
    document
      .getElementById("save-memb-api-key")
      .addEventListener("click", saveMemBApiKey);
    ...
    document
      .getElementById("toggle-memb-key")
      .addEventListener("click", () =>
        togglePasswordVisibility("memb-api-key")
      );
...
// Save memb API key to storage
async function saveMemBApiKey() {
  const apiKeyInput = document.getElementById("memb-api-key");
  const apiKey = apiKeyInput.value.trim();

  // Show loading status
  showStatus("Saving MemB API key...", "warning");

  try {
    // Send to background script for saving
    const response = await chrome.runtime.sendMessage({
      action: "saveConfig",
      config: { membApiKey: apiKey },
    });

    if (response.error) {
      showStatus(`Error: ${response.error}`, "error");
    } else {
      showStatus("MemB API key saved successfully", "success");
      loadConfig(); // Refresh the UI
    }
  } catch (error) {
    showStatus(`Error: ${error.message}`, "error");
...
async function loadConfig() {
  try {
    ...
    // Update memb API key field
    const membApiKeyInput = document.getElementById("memb-api-key");
    if (config.membApiKey) {
      membApiKeyInput.value = config.membApiKey;
      membApiKeyInput.type = "password"; // Ensure it's hidden by default
      document.getElementById("memb-api-key-section").style.display = "block";
      document.getElementById("memb-status-text").textContent = "Connected";
      document.getElementById("memb-status-text").style.color =
        "var(--success-color)";
    } else {
      membApiKeyInput.value = "";
      document.getElementById("memb-api-key-section").style.display = "block";
      document.getElementById("memb-status-text").textContent =
        "Not configured";
      document.getElementById("memb-status-text").style.color =
        "var(--warning-color)";
    }
