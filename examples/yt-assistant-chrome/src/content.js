...
const config = {
  ...
  membApiKey: "", // Will be set through extension options
};

// Initialize MemBAI - will be initialized properly when API key is available
let membclient = null;
let membInitializing = false;

// Function to initialize MemBAI with API key from storage
async function initializeMemBAI() {
  if (membInitializing) return; // Prevent multiple simultaneous initialization attempts
  membInitializing = true;

  try {
    // Get API key from storage
    const items = await chrome.storage.sync.get(["membApiKey"]);
    if (items.membApiKey) {
      try {
        // Create new client instance with v2.1.11 configuration
        membclient = new MemoryClient({
          apiKey: items.membApiKey,
        ...
        await membclient.updateProject({
...
Remember: The goal is to build a comprehensive understanding of both the user's knowledge and their learning journey through YouTube.`,
        ...
        console.error("Error initializing MemBAI:", error);
        return false;
      }
    } else {
      console.log("No MemBAI API key found in storage");
      return false;
    }
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await initializeMemBAI(); // Initialize MemBAI
});
...
async function sendMessage() {
  ...
  try {
    // If membclient is available, store the message as a memory and search for relevant memories
    if (membclient) {
      try {
        ...
        await membclient.add(
          ...
          {
            user_id: "youtube-assistant-memb", // Required parameter
            metadata: {
        ...
        const searchResults = await membclient.search(userMessage, {
          user_id: "youtube-assistant-memb", // Required parameter
          limit: 5,
        });
        ...
        console.error("Error with MemBAI operations:", memoryError);
...
async function loadMemories() {
  try {
    ...
    if (!membclient) {
      const initialized = await initializeMemBAI();
      if (!initialized) {
        memoriesContainer.innerHTML =
          '<div class="error">Please set your MemB API key in the extension options.</div>';
        return;
      }
    }

    const response = await membclient.getAll({
      user_id: "youtube-assistant-memb",
      page: 1,
      page_size: 50,
    });
