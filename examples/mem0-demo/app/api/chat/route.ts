...
import { addMemories, getMemories } from "@memb/vercel-ai-provider";
...
export async function POST(req: Request) {
  ...
  const membInstructions = retrieveMemories(memories);

  const result = streamText({
    model: openai("gpt-4o"),
    messages,
    // forward system prompt and tools from the frontend
    system: [SYSTEM_HIGHLIGHT_PROMPT, system, membInstructions].filter(Boolean).join("\n"),
