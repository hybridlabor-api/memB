...
def _config_path():
    return os.path.join(memb_dir, "config.json")


def _load_config():
    """Load ~/.memb/config.json, returning {} on missing/malformed file."""
    path = _config_path()
    ...
    try:
        ...
        _logger.debug("Failed to load memb config %s: %s", path, e)
        return {}


def _write_config(config):
    """Best-effort write of ~/.memb/config.json. Never raises."""
    path = _config_path()
    temp_path = None
    try:
        ...
        if temp_path:
            try:
                ...
                pass
        _logger.debug("Failed to write memb config %s: %s", path, e)


def setup_config():
    """Ensure ~/.memb/config.json exists with a top-level user_id.
...
def read_anon_ids():
    """Return anon IDs and alias markers from ~/.memb/config.json.
