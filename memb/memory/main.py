...
from memb.configs.base import MemoryConfig, MemoryItem
from memb.configs.enums import MemoryType
from memb.configs.prompts import (
    ADDITIVE_EXTRACTION_PROMPT,
    AGENT_CONTEXT_SUFFIX,
    PROCEDURAL_MEMORY_SYSTEM_PROMPT,
    generate_additive_extraction_prompt,
)
from memb.exceptions import LLMError
from memb.exceptions import ValidationError as MemBValidationError
from memb.memory.base import MemoryBase
from memb.memory.notices import (
...
from memb.utils.entity_extraction import extract_entities, extract_entities_batch
from memb.utils.factory import (
    EmbedderFactory,
    LlmFactory,
    RerankerFactory,
    VectorStoreFactory,
)
from memb.utils.lemmatization import lemmatize_for_bm25
from memb.utils.scoring import (
    ENTITY_BOOST_WEIGHT,
    get_bm25_params,
    normalize_bm25,
    score_and_rank,
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        if MEMB_TELEMETRY:
            ...
            telemetry_config_dict['collection_name'] = "membmigrations"

            # Set path for file-based vector stores
            telemetry_config = _safe_deepcopy_config(self.config.vector_store.config)
            if self.config.vector_store.provider in ["faiss", "qdrant"]:
                provider_path = f"migrations_{self.config.vector_store.provider}"
                telemetry_config_dict['path'] = os.path.join(memb_dir, provider_path)
        ...
        capture_event("memb.init", self, {"sync_type": "sync"})
    ...
    def add(
        ...
        *,
        ...
        Raises:
            MemBValidationError: If input validation fails (invalid memory_type, messages format, etc.).
    ...
    def get(self, memory_id):
        ...
        """
        capture_event("memb.get", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def get_all(
        ...
        **kwargs,
        ...
        capture_event(
            "memb.get_all", self, {"limit": limit, "keys": keys, "encoded_ids": encoded_ids, "sync_type": "sync"}
        )
    ...
    def update(
        ...
        """
        capture_event("memb.update", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def delete(self, memory_id):
        ...
        """
        capture_event("memb.delete", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def delete_all(self, user_id: Optional[str] = None, agent_id: Optional[str] = None, run_id: Optional[str] = None):
        ...
        keys, encoded_ids = process_telemetry_filters(filters)
        capture_event("memb.delete_all", self, {"keys": keys, "encoded_ids": encoded_ids, "sync_type": "sync"})
    ...
    def history(self, memory_id):
        ...
        """
        capture_event("memb.history", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def _create_procedural_memory(self, messages, metadata=None, prompt=None):
        ...
        capture_event("memb._create_procedural_memory", self, {"memory_id": memory_id, "sync_type": "sync"})
    ...
    def reset(self):
        ...
        capture_event("memb.reset", self, {"sync_type": "sync"})
...
class AsyncMemory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        if MEMB_TELEMETRY:
            ...
            if self.config.vector_store.provider in ["faiss", "qdrant"]:
                provider_path = f"migrations_{self.config.vector_store.provider}"
                telemetry_config.path = os.path.join(memb_dir, provider_path)
        ...
        capture_event("memb.init", self, {"sync_type": "async"})
    ...
    async def get(self, memory_id):
        ...
        Returns:
            dict: Retrieved memory.
        """
        capture_event("memb.get", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def get_all(
        ...
        **kwargs,
        ...
        keys, encoded_ids = process_telemetry_filters(effective_filters)
        capture_event(
            "memb.get_all", self, {"limit": limit, "keys": keys, "encoded_ids": encoded_ids, "sync_type": "async"}
        )
    ...
    async def update(
        ...
        Example:
            ...
            {'message': 'Memory updated successfully!'}
        """
        capture_event("memb.update", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def delete(self, memory_id):
        ...
        Args:
            memory_id (str): ID of the memory to delete.
        """
        capture_event("memb.delete", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def delete_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        keys, encoded_ids = process_telemetry_filters(filters)
        capture_event("memb.delete_all", self, {"keys": keys, "encoded_ids": encoded_ids, "sync_type": "async"})
    ...
    async def history(self, memory_id):
        ...
        Returns:
            list: List of changes for the memory.
        """
        capture_event("memb.history", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def _create_procedural_memory(self, messages, metadata=None, llm=None, prompt=None):
        ...
        capture_event("memb._create_procedural_memory", self, {"memory_id": memory_id, "sync_type": "async"})
    ...
    async def reset(self):
        ...
        capture_event("memb.reset", self, {"sync_type": "async"})
