...
from memb.configs.prompts import MEMORY_ANSWER_PROMPT
from memb.memory.telemetry import capture_client_event, capture_event
...
class MemB:
    def __init__(
        ...
        if api_key:
            self.memb_client = MemoryClient(api_key, host)
        else:
            self.memb_client = Memory.from_config(config) if config else Memory()

        self.chat = Chat(self.memb_client)


class Chat:
    def __init__(self, memb_client):
        self.completions = Completions(memb_client)


class Completions:
    def __init__(self, memb_client):
        self.memb_client = memb_client
    ...
    def create(
        ...
        if isinstance(self.memb_client, Memory):
            capture_event("memb.chat.create", self.memb_client)
        else:
            capture_client_event("memb.chat.create", self.memb_client)
        return response
    ...
    def _fetch_relevant_memories(self, messages, user_id, agent_id, run_id, filters, top_k):
        ...
        return self.memb_client.search(
            query="\n".join(message_input),
            user_id=user_id,
            agent_id=agent_id,
            run_id=run_id,
    ...
    def _format_query_with_memories(self, messages, relevant_memories):
        # Check if self.memb_client is an instance of Memory or MemoryClient

        entities = []
        if isinstance(self.memb_client, memb.memory.main.Memory):
        ...
        elif isinstance(self.memb_client, memb.client.main.MemoryClient):
