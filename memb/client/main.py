...
from memb.client.project import AsyncProject, Project
from memb.client.types import (
    AddMemoryOptions,
...
from memb.client.utils import api_error_handler

# Exception classes are referenced in docstrings only
from memb.memory.setup import get_user_id, is_aliased, mark_aliased, read_anon_ids, setup_config
from memb.memory.telemetry import capture_client_event, client_telemetry
...
class MemoryClient:
    """Client for interacting with the MemB API.

    This class provides methods to create, retrieve, search, and delete
    memories using the MemB API.

    Attributes:
        api_key (str): The API key for authenticating with the MemB API.
        host (str): The base URL for the MemB API.
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("MemB API Key not provided. Please provide an API Key.")
    ...
    def chat(self):
        """Start a chat with the MemB AI. (Not implemented)
...
class AsyncMemoryClient:
    """Asynchronous client for interacting with the MemB API.
