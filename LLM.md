...
**Import:** `from memb import Memory, AsyncMemory`

#### Initialization
```python
from memb import Memory
from memb.configs.base import MemoryConfig
...
**Import:** `from memb import MemoryClient, AsyncMemoryClient`
...
config = MemoryConfig(
    ...
    history_db_path="~/.memb/history.db",
    version="v1.1",
...
```python
# MemBTeachability lives in examples/notebooks/helper/ — see examples/notebooks/memb-autogen.ipynb
from helper.memb_teachability import MemBTeachability
from memb import Memory

# Add memory capability to AutoGen agents
memory = Memory()
teachability = MemBTeachability(
    verbosity=1,
    reset_db=False,
...
class MemBLangChainMemory(ConversationBufferMemory):
    ...
    def save_context(self, inputs, outputs):
        # Save to both LangChain and MemB
        super().save_context(inputs, outputs)
        
        # Store in MemB for long-term memory
        self.memb.add([
    ...
    def load_memory_variables(self, inputs):
        ...
        relevant_memories = self.memb.search(
            str(inputs),
            user_id=self.user_id,
            limit=3
        )
...
- **GitHub Repository**: https://github.com/membai/memb
- **Discord Community**: https://memb.dev/DiG
- **Platform**: https://app.memb.ai?utm_source=oss&utm_medium=llm
- **Research Paper**: https://memb.ai/research
- **Examples**: https://github.com/membai/memb/tree/main/examples

## License

MemB is available under the Apache 2.0 License. See the [LICENSE](https://github.com/membai/memb/blob/main/LICENSE) file for more details.
