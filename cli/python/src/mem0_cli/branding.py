"""Branding and ASCII art for memb CLI."""
...
def print_error(console: Console, message: str, hint: str | None = None) -> None:
    from memb_cli.state import get_current_command, is_agent_mode
        ...
        if ctx.error_msg:
            ...
            if "Authentication failed" in ctx.error_msg:
                _err.print(
                    ...
                    f" · [bold]https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=cli-python[/bold][/]"
                )
        raise
    else:
        elapsed = time.perf_counter() - start
