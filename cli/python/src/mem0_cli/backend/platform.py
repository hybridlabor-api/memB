"""Platform (SaaS) backend — communicates with api.memb.ai."""
...
from memb_cli.backend.base import Backend
from memb_cli.config import PlatformConfig
...
class PlatformBackend(Backend):
    """Backend that talks to the memb Platform API."""
    ...
    def __init__(self, config: PlatformConfig) -> None:
        ...
        self._client = httpx.Client(
            ...
            headers={
                ...
                "X-MemB-Client-Version": __version__,
            },
            timeout=30.0,
        )

    def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        from memb_cli.state import capture_notice, is_agent_mode

        self._client.headers["X-MemB-Caller-Type"] = "agent" if is_agent_mode() else "user"
        ...
        notice = None
        if isinstance(data, dict) and "memb_notice" in data:
            notice = data.pop("memb_notice")
        elif (
            isinstance(data, list)
            and data
            and isinstance(data[0], dict)
            and "memb_notice" in data[0]
        ):
            notice = data[0].pop("memb_notice")
        if notice is None:
            notice = resp.headers.get("X-MemB-Notice-Message") or None
