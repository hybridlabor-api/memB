"""Configuration management for memb CLI.
...
CONFIG_DIR = Path.home() / ".memb"
CONFIG_FILE = CONFIG_DIR / "config.json"

DEFAULT_BASE_URL = "https://api.memb.ai"
CONFIG_VERSION = 1
...
class AgentRushConfig:
    ...
    # Empty until first interactive `memb agent-rush add`.
    acknowledged_at: str = ""
...
def ensure_config_dir() -> Path:
    """Create ~/.memb directory with secure permissions if it doesn't exist."""
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    os.chmod(CONFIG_DIR, stat.S_IRWXU)  # 0700
    return CONFIG_DIR


def load_config() -> MemBConfig:
...
def save_config(config: MemBConfig) -> None:
    ...
    if config.platform.api_key:
        try:
            from memb_cli.plugin_sync import sync_api_key
...
def get_nested_value(config: MemBConfig, dotted_key: str) -> Any:
...
def set_nested_value(config: MemBConfig, dotted_key: str, value: str) -> bool:
