...
Usage: python -m memb_cli.telemetry_sender   (JSON context is read from stdin;
...
2. Caches the email in ~/.memb/config.json for future runs
...
def main() -> None:
    ...
    if ctx.get("needs_email") and ctx.get("memb_api_key"):
...
def _resolve_and_cache_email(ctx: dict, payload: dict) -> None:
    ...
    try:
        ping_url = ctx["memb_base_url"].rstrip("/") + "/v1/ping/"
        req = urllib.request.Request(
            ping_url,
            headers={
                "Authorization": "Token " + ctx["memb_api_key"],
