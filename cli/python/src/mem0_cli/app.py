"""Main CLI application — the entrypoint for `memb`."""
...
from memb_cli import __version__
from memb_cli.branding import BRAND_COLOR, print_error, print_warning
...
app = typer.Typer(
    name="memb",
    help=f"◆ MemB CLI v{__version__} · Python SDK\n\n   The Memory Layer for AI Agents",
...
config_app = typer.Typer(
    name="config",
    help="Manage memb configuration.",
    no_args_is_help=True,
    rich_markup_mode="rich",
)
...
def _fire_telemetry(command_name: str, extra: dict | None = None) -> None:
    ...
    try:
        from memb_cli.telemetry import capture_event
...
def _get_backend_and_config(
    ...
    from memb_cli.backend import get_backend
    from memb_cli.backend.platform import AuthError
    from memb_cli.config import load_config, save_config
...
def _stdin_is_piped() -> bool:
    ...
    from memb_cli.state import is_agent_mode
...
def main_callback(
    ...
    if json_agent:
        from memb_cli.state import set_agent_mode

        set_agent_mode(True)
    if version:
        from memb_cli.commands.utils import cmd_version
    ...
    if ctx.invoked_subcommand:
        ...
        from memb_cli.state import set_current_command
...
def add(
    ...
    """
    from memb_cli.commands.memory import cmd_add
...
def search(
    ...
    """
    from memb_cli.commands.memory import cmd_search
...
def get(
    ...
    """
    from memb_cli.commands.memory import cmd_get
...
def list_cmd(
    ...
    """
    from memb_cli.commands.memory import cmd_list
...
def update(
    ...
    Examples:
      memb update abc-123-def-456 "new text"
      memb update abc-123 --metadata '{{"key":"val"}}'
      echo "new text" | memb update abc-123
    """
    from memb_cli.commands.memory import cmd_update
...
def delete(
    ...
    if modes == 0:
        print_error(
            ...
            hint="Run 'memb delete --help' for usage.",
        )
        raise typer.Exit(1)
    ...
    if memory_id is not None:
        ...
        from memb_cli.commands.memory import cmd_delete
        ...
        from memb_cli.commands.memory import cmd_delete_all
        ...
        from memb_cli.commands.entities import cmd_entities_delete
...
def config_show(
    ...
    """
    from memb_cli.commands.config_cmd import cmd_config_show
...
def config_get(
    ...
    """
    from memb_cli.commands.config_cmd import cmd_config_get
...
def config_set(
    ...
    Examples:
      ...
      memb config set platform.base_url https://custom.api.memb.ai
    """
    from memb_cli.commands.config_cmd import cmd_config_set
...
def entity_list(
    ...
    """
    from memb_cli.commands.entities import cmd_entities_list
...
def entity_delete(
    ...
    Examples:
      ...
      memb entity delete -u alice --dry-run
    """
    from memb_cli.commands.entities import cmd_entities_delete
...
def event_list(
    ...
    Examples:
      memb event list
      memb event list -o json
    """
    from memb_cli.commands.events_cmd import cmd_event_list
...
def event_status(
    ...
    Examples:
      ...
      memb event status <event-id> -o json
    """
    from memb_cli.commands.events_cmd import cmd_event_status
...
def init(
    ...
    Examples:
      ...
      memb init --agent --agent-caller claude-code   # AI agent self-identifies on Agent Mode bootstrap
      memb init --email alice@company.com  # Claims an existing Agent Mode key when one is present
    """
    from memb_cli.commands.init_cmd import run_init
...
def identify(
    ...
    Run this once after `memb init --agent` if you didn't pass --agent-caller.
    Idempotent — re-running just overwrites the value.

    Example:
      memb identify claude-code
    """
    from memb_cli.commands.identify_cmd import run_identify
...
def whoami_cmd() -> None:
    ...
    Example:
      memb whoami
    """
    from memb_cli.commands.whoami_cmd import run_whoami
...
def agent_rush_add(
    ...
    Example:
      memb agent-rush add "I enjoy solving constraint-satisfaction problems."
    """
    from memb_cli.commands.agent_rush_cmd import run_agent_rush_add
...
def agent_rush_search(
    ...
    Example:
      memb agent-rush search "constraint satisfaction"
    """
    from memb_cli.commands.agent_rush_cmd import run_agent_rush_search
...
def status(
    ...
    Examples:
      ...
      memb status -o json
    """
    from memb_cli.commands.utils import cmd_status
...
def import_cmd(
    ...
    """
    from memb_cli.commands.utils import cmd_import
...
def _build_help_json() -> dict:
    ...
    commands = {
        ...
        "init": {
            "description": "Interactive setup wizard for memb CLI.",
            "usage": "memb init",
            "options": {
    ...
    return {
        ...
        "help": {
            "human": "memb <command> --help    Get help for a command",
            "machine": "memb help --json         Machine-readable help (for LLM agents)",
        },
    }
...
def help(
    ...
    if json:
        ...
        console.print(
            f"[{BRAND_COLOR}]◆ memb CLI[/] v{__version__} — The Memory Layer for AI Agents\n"
        )
        console.print("Usage: memb <command> [OPTIONS]\n")
        ...
        console.print()
        console.print("  memb <command> --help    Get help for a command")
        console.print("  memb help --json         Machine-readable help (for LLM agents)")
        console.print()
...
def main() -> None:
    ...
    # Special case: `memb init --agent` is a subcommand flag (Agent Mode bootstrap)
    ...
    try:
        ...
        from memb_cli.state import is_agent_mode, take_notice
