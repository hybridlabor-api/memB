"""memb identify — declare which agent owns the current agent-mode key.

Used when `memb init --agent` ran without --agent-caller, so the backend
saved agent_caller=NULL. The agent re-runs `memb identify <name>` to PATCH
...
def run_identify(name: str) -> None:
    ...
    if not config.platform.api_key:
        print_error(
            err_console,
            "No API key configured. Run `memb init --agent` first.",
        )
        raise typer.Exit(1)
