"""memb agent-rush — AGENTRUSH game commands.
...
from memb_cli.branding import print_error, print_success
...
_SOURCE_HEADERS = {
    ...
    "X-MemB-Mode": "agent-rush",
}

_ERROR_HINTS = {
    "agentrush_search_first": "Run 3 'memb agent-rush search' commands before adding.",
    "agentrush_search_quota": "You've used your 3 lifetime searches.",
    "agentrush_add_quota": "You've used your 3 lifetime adds.",
    "agentrush_not_agent_mode": "Re-run 'memb init --agent' to bootstrap an agent-mode key.",
...
def _call(path: str, body: dict) -> dict:
    ...
    if not config.platform.api_key:
        print_error(err_console, "Not initialized. Run `memb init --agent` first.")
        raise typer.Exit(1)
