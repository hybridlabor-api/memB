...
def cmd_status(
    ...
    *,
    ...
    from memb_cli.output import format_agent_envelope
    from memb_cli.state import is_agent_mode, set_current_command
    ...
    if result.get("error"):
        ...
        if "Authentication failed" in str(result["error"]):
            lines.append("")
            lines.append(
                f"  [{DIM_COLOR}]Run [bold]memb init[/bold] to reconfigure your API key[/]"
            )
            lines.append(
                f"  [{DIM_COLOR}]Get a key at [bold]https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=cli-python[/bold][/]"
            )
...
def cmd_version() -> None:
    """Show version."""
    console.print(f"  [{BRAND_COLOR}]◆ MemB[/] CLI v{__version__}")
