...
from memb_cli.branding import BRAND_COLOR, print_error, print_info
...
def run_whoami() -> None:
    ...
    if not session_id:
        print_error(
            err_console,
            "No default_user_id found. Run `memb init --agent` first.",
        )
        raise typer.Exit(1)
    console.print(f"Your AGENTRUSH identifier:  [{BRAND_COLOR}]{session_id}[/{BRAND_COLOR}]")
    print_info(console, "Find your row at https://memb.ai/agentrush")
