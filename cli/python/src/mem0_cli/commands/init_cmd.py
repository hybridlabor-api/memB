"""memb init — interactive setup wizard."""
        ...
        if not code:
            ...
            if not sys.stdin.isatty():
                print_error(
                    ...
                    hint="Run: memb init --email <email> --code <code>",
                )
                raise typer.Exit(1)
            console.print()
    ...
    *,
    ...
    Claim dispatch:
      - If `--email` is set AND existing config has `agent_mode=true`, run the
        ...
        email-based key.
    """
    from memb_cli.agent_detect import detect_agent_caller
    from memb_cli.commands.agent_mode_cmd import bootstrap_via_backend, claim_via_otp
    from memb_cli.state import is_agent_mode as _global_agent_mode
    ...
    if not api_key and not email and _agent_ctx:
        from memb_cli.output import format_json_envelope
        from memb_cli.state import is_agent_mode as _is_json_mode
            ...
            if _is_json_mode():
                format_json_envelope(
                    ...
                    data={
                        ...
                        "agent_mode": False,
                        "message": "Existing MemB API key found and reused. No Agent Mode key was created.",
                    },
                )
            else:
                msg = (
    ...
    if not force and CONFIG_FILE.exists():
        ...
        if existing.platform.api_key:
            from memb_cli.config import redact_key
    ...
    if email:
        ...
        config.defaults.user_id = (
            user_id or os.environ.get("USER") or os.environ.get("USERNAME") or "memb-cli"
        )

        save_config(config)

        console.print()
        print_success(console, "Authenticated! Configuration saved to ~/.memb/config.json")
        console.print()
        console.print(f"  [{DIM_COLOR}]Get started:[/]")
        console.print(f'  [{DIM_COLOR}]  memb add "I prefer dark mode"[/]')
        console.print(f'  [{DIM_COLOR}]  memb search "preferences"[/]')
        console.print()
        return
    ...
    if not sys.stdin.isatty():
        if not api_key:
            print_error(
                ...
                hint="Run: memb init --api-key <key>, --email <addr>, or --agent for unattended Agent Mode bootstrap.",
            )
            raise typer.Exit(1)
        user_id = user_id or os.environ.get("USER") or os.environ.get("USERNAME") or "memb-cli"
    ...
    if api_key and user_id:
        ...
        save_config(config)
        print_success(console, "Configuration saved to ~/.memb/config.json")
        return

    print_banner(console)
    console.print()
    print_info(console, "Welcome! Let's set up your memb CLI.\n")
    ...
    if config.defaults.user_id:
        ...
        console.print(f'  [{DIM_COLOR}]  memb add "I prefer dark mode" --user-id alice[/]')
        console.print(f'  [{DIM_COLOR}]  memb search "preferences" --user-id alice[/]')
    console.print()


def _setup_platform(config: MemBConfig) -> None:
    """Platform setup flow."""
    console.print()
    console.print(
        f"  [{DIM_COLOR}]Get your API key at https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=cli-python[/]"
    )
    console.print()
...
def _setup_defaults(config: MemBConfig) -> None:
    """Collect default entity IDs."""
    console.print()
    print_info(console, "Set default entity IDs (press Enter to skip).\n")

    _default_user = os.environ.get("USER") or os.environ.get("USERNAME") or "memb-cli"
    user_id = Prompt.ask(
...
def _validate_platform(config: MemBConfig) -> None:
    ...
    try:
        ...
        if status.get("connected"):
            print_success(console, "Connected to memb Platform!")
            ...
            print_error(
                ...
                hint="Visit https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=cli-python to get a new key, then run memb init again.",
            )
