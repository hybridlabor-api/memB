...
def _get_or_create_anonymous_id() -> str:
    ...
    Stored in ~/.memb/config.json under `telemetry.anonymous_id` so that
...
def _get_distinct_id() -> str:
    ...
    """
    try:
        from memb_cli.config import load_config
...
def capture_event(
    ...
    try:
        ...
        from memb_cli.config import CONFIG_FILE, load_config, save_config
        ...
        context = {
            ...
            "memb_api_key": config.platform.api_key or "",
            "memb_base_url": config.platform.base_url or "https://api.memb.ai",
            "config_path": str(CONFIG_FILE),
            "anon_distinct_id_to_alias": anon_id_to_alias,
        }

        child = subprocess.Popen(
            [sys.executable, "-m", "memb_cli.telemetry_sender"],
