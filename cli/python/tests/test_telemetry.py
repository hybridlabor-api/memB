...
from memb_cli.telemetry_sender import _load_context
...
def test_capture_event_writes_context_to_stdin_not_argv(isolate_config, monkeypatch):
    ...
    monkeypatch.setattr("memb_cli.telemetry.subprocess.Popen", fake_popen)

    capture_event("unit_test_event", {"case": "stdin-secret"})

    argv = captured["args"]
    assert argv == [sys.executable, "-m", "memb_cli.telemetry_sender"]
    ...
    assert ctx["memb_api_key"] == "m0-test-secret"
