"""Parity tests for `memb init --agent` (Agent Mode bootstrap).
...
def _run(args: list[str], home_dir: str | None = None) -> subprocess.CompletedProcess:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "memb_cli", *args],
        capture_output=True,
        encoding="utf-8",
        env=env,
        timeout=15,
...
class TestInitFlagSurface:
    """`memb init --help` must expose the Agent Mode flags."""
...
class TestArgvPreprocessing:
    ...
    subcommand, making ``memb init --agent`` indistinguishable from a
    plain ``memb init`` (interactive wizard).
    """
    ...
    def test_init_with_agent_reaches_subcommand(self, clean_home):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "memb_cli", "init", "--agent"],
            capture_output=True,
            encoding="utf-8",
            env={
...
class TestJsonEnvelopeParity:
    """`memb init --agent --json` should produce a JSON envelope on success.
    ...
    def test_init_agent_json_no_traceback_on_network_failure(self, clean_home):
        result = subprocess.run(
            [sys.executable, "-m", "memb_cli", "init", "--agent", "--json"],
            capture_output=True,
            encoding="utf-8",
            env={
                **{k: v for k, v in os.environ.items() if not k.startswith("MEMB_")},
...
class TestInitInCommandList:
    """`memb --help` must list `init` so agents walking the top-level help
