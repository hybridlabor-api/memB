...
def clean_home(tmp_path):
    """Return a temp directory to use as HOME, ensuring no ~/.memb exists."""
    return str(tmp_path)
...
class TestCLIIsolated:
    ...
    def test_add_no_key_errors(self, clean_home):
        """Without an API key, `memb add` must fail with a helpful message."""
        result = _run(
    ...
    def test_search_no_key_errors(self, clean_home):
        """Without an API key, `memb search` must fail."""
        result = _run(
    ...
    def test_list_no_key_errors(self, clean_home):
        """Without an API key, `memb list` must fail."""
