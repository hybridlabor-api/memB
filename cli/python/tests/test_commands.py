...
from memb_cli.commands.config_cmd import (
    cmd_config_get,
    cmd_config_set,
    cmd_config_show,
)
from memb_cli.commands.entities import cmd_entities_delete, cmd_entities_list
from memb_cli.commands.events_cmd import cmd_event_list, cmd_event_status
from memb_cli.commands.memory import (
    cmd_add,
    cmd_delete,
    cmd_delete_all,
    cmd_get,
...
from memb_cli.commands.utils import (
    cmd_import,
    cmd_status,
)
...
class TestAddCommand:
    def test_add_text(self, mock_backend):
        ...
        with (
            patch("memb_cli.commands.memory.console", console),
            patch("memb_cli.commands.memory.err_console", err_console),
        ):
            cmd_add(
                mock_backend,
                "I prefer dark mode",
    ...
    def test_add_no_content_exits(self, mock_backend):
        ...
        with (
            patch("memb_cli.commands.memory.console", console),
            patch("memb_cli.commands.memory.err_console", err_console),
            patch("memb_cli.commands.memory._stdin_is_piped", return_value=False),
...
class TestStatusCommand:
    def test_status_connected(self, mock_backend):
        ...
        err_console, _err_buf = _make_err_console()
        with (
            patch("memb_cli.commands.utils.console", console),
            patch("memb_cli.commands.utils.err_console", err_console),
        ):
...
class TestEntitiesListCommand:
    def test_list_users(self, mock_backend):
        ...
        err_console, _err_buf = _make_err_console()
        with (
            patch("memb_cli.commands.entities.console", console),
            patch("memb_cli.commands.entities.err_console", err_console),
        ):
...
class TestConfigCommands:
    def test_config_show(self, isolate_config):
        from memb_cli.config import MemBConfig, save_config
        ...
        err_console, _err_buf = _make_err_console()
        with (
            patch("memb_cli.commands.config_cmd.console", console),
            patch("memb_cli.commands.config_cmd.err_console", err_console),
        ):
            cmd_config_show()
        output = buf.getvalue()
    ...
    def test_config_set_and_get(self, isolate_config):
        ...
        with (
            patch("memb_cli.commands.config_cmd.console", console1),
            patch("memb_cli.commands.config_cmd.err_console", err_console1),
        ):
            cmd_config_set("platform.base_url", "https://custom.api.memb.ai")

        console2, buf2 = _make_console()
        err_console2, _err_buf2 = _make_err_console()
        with (
            patch("memb_cli.commands.config_cmd.console", console2),
            patch("memb_cli.commands.config_cmd.err_console", err_console2),
        ):
            cmd_config_get("platform.base_url")
        output = buf2.getvalue()
        assert "custom.api.memb.ai" in output
...
class TestEventCommands:
    def test_event_list_table(self, mock_backend):
        ...
        err_console, _err_buf = _make_err_console()
        with (
            patch("memb_cli.commands.events_cmd.console", console),
            patch("memb_cli.commands.events_cmd.err_console", err_console),
        ):
...
class TestAgentMode:
    ...
    def test_no_spinner_in_agent_mode(self, mock_backend):
        ...
        with (
            patch("memb_cli.commands.memory.console", console),
            patch("memb_cli.commands.memory.err_console", err_console_buf),
        ):
            cmd_search(
                mock_backend,
                "query",
