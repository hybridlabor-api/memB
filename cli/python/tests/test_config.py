...
class TestConfig:
    def test_default_config(self):
        config = MemBConfig()
        assert config.platform.base_url == "https://api.memb.ai"
    ...
    def test_config_file_permissions(self, isolate_config):
        ...
        from memb_cli.config import CONFIG_FILE
    ...
    def test_backward_compat_no_defaults_key(self, isolate_config):
        ...
        from memb_cli.config import CONFIG_FILE, ensure_config_dir

        ensure_config_dir()
        # Write a config without the "defaults" key
        data = {
            "version": 1,
            "platform": {"api_key": "m0-test", "base_url": "https://api.memb.ai"},
        }
...
class TestResolveIds:
    def test_cli_flag_overrides_default(self):
        from memb_cli.app import _resolve_ids
