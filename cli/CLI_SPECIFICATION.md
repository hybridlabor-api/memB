# memb CLI SDK Specification

Complete reference for the memb CLI. This document is the authoritative guide for any developer or AI agent working on this SDK.
...
memb CLI is the official command-line interface for [memb](https://memb.ai) -- the memory layer for AI agents. It lets developers and AI agents add, search, list, update, and delete memories via the memb Platform API from the terminal.

### Who is it for?

- Developers integrating memb into their workflows
...
The `cli/` directory provides the memb CLI in two languages with a shared specification for behavioral consistency.

| Language   | Directory  | Package Name  | Install Command            |
|------------|------------|---------------|----------------------------|
| TypeScript | `node/`    | `@memb/cli`   | `npm install -g @memb/cli` |
| Python     | `python/`  | `memb-cli`    | `pip install memb-cli`     |

Both implementations produce a binary named `memb` and provide **identical CLI behavior** -- same commands, same options, same output formats, same error messages.
...
| Entrypoint       | `memb = "memb_cli.app:main"`        | `"bin": { "memb": "./dist/index.js" }`  |
...
Interactive setup wizard for memb CLI.
...
- If `~/.memb/config.json` exists with an API key, the CLI warns and asks for confirmation before overwriting.
...
- Interactive mode: prints banner, prompts for API key (masked with `*`), prompts for default user ID (default: `memb-cli`), validates connection, saves config.
...
echo "text" | memb add -u alice
memb add --file msgs.json -u alice -o json
```
...
memb update abc-123 --metadata '{"key":"val"}'
...
1. **Single memory:** `memb delete <memory_id>` -- deletes one memory by ID. Cannot combine with `--all` or `--entity`.
2. **Bulk delete:** `memb delete --all [scope]` -- deletes all memories matching scope filters. Use `--project` with `--all` to wipe all memories project-wide (sends wildcard `*` entity IDs). Cannot combine with `<memory_id>` or `--entity`.
3. **Entity cascade:** `memb delete --entity [scope]` -- deletes the entity itself and all its memories. Cannot combine with `<memory_id>` or `--all`.
...
**Examples:**
```bash
memb delete abc-123-def-456              # single memory
memb delete --all -u alice --force        # all memories for user
memb delete --all --project --force       # project-wide wipe
memb delete --entity -u alice --force     # entity + all its memories
memb delete abc-123 --dry-run             # preview single delete
```
...
| Property         | Value |
|------------------|-------|
| Usage            | `memb entity list <entity_type>` |
| needsBackend     | Yes |
| needsConfig      | No |
| resolveIds       | No |
| resolveGraph     | No |
...
Delete an entity and ALL its memories (cascade). Also accessible via `memb delete --entity`.
...
- Directory: `~/.memb/` (created with permissions `0700`)
- File: `~/.memb/config.json` (written with permissions `0600`)
...
| `platform.api_key`       | string  | `""`                     | API key for memb Platform. |
...
- Config directory (`~/.memb/`): created with mode `0o700` (owner read+write+execute only).
- Config file (`~/.memb/config.json`): written with mode `0o600` (owner read+write only).
...
When both `--api-key` and `--user-id` are provided to `memb init`:
...
Every data-returning command supports `--output json` (or `--json` for `help`). This enables machine consumption by AI agents and scripts. JSON output goes to stdout while human-readable spinners/timing go to stderr, so piping `memb list -o json | jq .` works cleanly.
...
cat query.txt | memb search -u alice
echo "updated text" | memb update abc-123
```
...
```json
{
  ...
  "usage": "memb my-command <arg> [OPTIONS]",
  "needsBackend": true,
  "needsConfig": true,
  "resolveIds": true,
  "resolveGraph": false,
...
**Python** (`python/src/memb_cli/backend/base.py` and `platform.py`):
...
**Python** (`python/src/memb_cli/commands/`):
...
**Python** (`python/src/memb_cli/app.py`):
```python
...
def my_command(
    ...
    """Command description."""
    from memb_cli.commands.my_module import cmd_my_command
...
| Module        | Purpose |
|---------------|---------|
| **config**    | Load `~/.memb/config.json`, apply env var overrides, enforce precedence. Implement `load_config`, `save_config`, `ensure_config_dir`, `redact_key`, `get_nested_value`, `set_nested_value`. |
...
- [ ] Binary must be named `memb`

### 10.3 Package Metadata

Follow the naming conventions:
- Package description: "The official CLI for memb -- the memory layer for AI agents"
- Author: `memb.ai <founders@memb.ai>`
- License: Apache-2.0
- Keywords: `memb`, `memory`, `ai`, `agents`, `cli`
