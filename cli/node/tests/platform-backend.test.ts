/**
 * Tests for the Platform backend (memb Platform API client).
 */
...
describe("PlatformBackend path encoding", () => {
  it("encodes memory IDs before interpolating them into paths", async () => {
    ...
    expect(urls).toEqual([
      "https://api.memb.ai/v1/memories/mem%2Fa%3Fb%23c/?source=CLI",
      "https://api.memb.ai/v1/memories/mem%2Fa%3Fb%23c/",
      "https://api.memb.ai/v1/memories/mem%2Fa%3Fb%23c/?source=CLI",
    ]);
  });
  ...
  it("encodes entity and event IDs before interpolating them into paths", async () => {
    ...
    const urls = fetchMock.mock.calls.map((call) => call[0]);
    expect(urls).toEqual([
      "https://api.memb.ai/v2/entities/user/org%2Fteam%3Factive%23frag/?source=CLI",
      "https://api.memb.ai/v1/event/evt%2Fa%3Fb%23c/",
    ]);
  });
});
