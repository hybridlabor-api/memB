...
import type { MemBConfig } from "./config.js";
...
async function getBackendAndConfig(
	apiKey?: string,
	baseUrl?: string,
): Promise<{ backend: Backend; config: MemBConfig }> {
...
program
	.name("memb")
	.description(
		`◆ MemB CLI v${CLI_VERSION} · Node.js SDK\n\nThe Memory Layer for AI Agents`,
	)
	// Positional options: flags AFTER a subcommand name belong to that
	// subcommand, not the global program. Without this, `memb init --agent`
	...
	.on("option:version", () => {
		console.log(`  ${colors.brand("◆ MemB")} CLI v${CLI_VERSION}`);
		process.exit(0);
	})
	.option("--json", "Output as JSON for agent/programmatic use.")
	.option(
		"--agent",
		"Output as JSON for agent/programmatic use. (alias: --json) Place BEFORE the subcommand: `memb --agent <cmd>`. On `init`, `memb init --agent` is the Agent Mode bootstrap flag instead.",
	)
...
program
	...
	// `memb init --agent --json` works without requiring users to move it
	...
	.addHelpText(
		"after",
		"\nExamples:\n  $ memb init\n  $ memb init --api-key m0-xxx --user-id alice\n  $ memb init --email you@example.com\n  $ memb init --email you@example.com --code 123456\n  $ memb init --agent             # Bootstrap an Agent Mode account (unattended)\n  $ memb init --email you@example.com  # Claims an existing Agent Mode key when one is present",
	)
	.action(async (opts) => {
...
agentRush
	...
	.addHelpText(
		"after",
		'\nExamples:\n  $ memb agent-rush add "I used memb to build a coding agent"\n  $ memb agent-rush add "Agents that remember are better agents"',
	)
...
agentRush
	...
	.addHelpText(
		"after",
		'\nExamples:\n  $ memb agent-rush search "agents and memory and tools"\n  $ memb agent-rush search "coding assistant"',
	)
	.action(async (parts: string[]) => {
		...
		if (!memoryId && !opts.all && !opts.entity) {
			printError(
				"Specify a memory ID, --all, or --entity.\n" +
					"  memb delete <id>              Delete a single memory\n" +
					"  memb delete --all [scope]     Delete all memories matching scope\n" +
					"  memb delete --entity [scope]  Delete an entity and all its memories",
			);
			process.exit(1);
		}
...
const configCmd = program
	.command("config")
	.description("Manage memb configuration.")
	.addHelpCommand(false);
...
program
	...
	.addHelpText("after", "\nExamples:\n  $ memb help\n  $ memb help --json")
	.action((opts) => {
		// opts.json is set when `memb help --json` is used (subcommand flag).
		...
		if (opts.json || program.opts().json) {
			...
			console.log(
				`${b("◆ MemB CLI")} v${CLI_VERSION} · Node.js SDK\n  The Memory Layer for AI Agents\n`,
			);
			console.log("Usage: memb <command> [OPTIONS]\n");
			...
			console.log();
			console.log("  memb <command> --help    Get help for a command");
			console.log(
				"  memb help --json         Machine-readable help (for LLM agents)",
			);
			console.log();
		}
	});
