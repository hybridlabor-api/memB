/**
 * `memb agent-rush <add|search> "..."` — wraps the AGENTRUSH platform endpoints.
...
const ERROR_HINTS: Record<string, string> = {
	agentrush_search_first:
		"Run 3 'memb agent-rush search' commands before adding.",
	...
	agentrush_not_agent_mode:
		"Re-run 'memb init --agent' to bootstrap an agent-mode key.",
...
async function callEndpoint(
	...
	if (!config.platform?.apiKey) {
		printError("Not initialized. Run `memb init --agent` first.");
		process.exit(1);
	}
