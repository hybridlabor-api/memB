...
export async function cmdStatus(
	...
	if (result.error) {
		...
		if (String(result.error).includes("Authentication failed")) {
			lines.push("");
			lines.push(
				`  ${dim("Run")} ${brand("memb init")} ${dim("to reconfigure your API key")}`,
			);
			lines.push(
				`  ${dim("Get a key at")} ${brand("https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=cli-node")}`,
			);
		}
	}
...
export function cmdVersion(): void {
	console.log(`  ${brand("◆ MemB")} CLI v${CLI_VERSION}`);
}
