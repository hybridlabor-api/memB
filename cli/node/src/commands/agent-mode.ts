...
import { type MemBConfig, saveConfig } from "../config.js";
 ...
 *
 * @param config - MemBConfig mutated in place with the new platform values.
 ...
 *   flag; the agent can backfill later via `memb identify <name>`. Sent to the
...
export async function bootstrapViaBackend(
	...
	const baseUrl = (config.platform.baseUrl || "https://api.memb.ai").replace(
		/\/+$/,
		"",
	);
	...
	try {
		...
		printError(
			`Network error contacting MemB: ${err instanceof Error ? err.message : String(err)}`,
		);
		process.exit(1);
	}
	...
	if (envelope.memb_notice) {
		console.log(`\n\x1b[33m🔔 ${envelope.memb_notice}\x1b[0m\n`);
	} else {
		// Fallback for older backends without the unified notice field.
		const claimCmd = envelope.claim_command ?? "memb init --email <your-email>";
		console.log(`  ${dim(`To claim this account later: ${claimCmd}`)}`);
	}

	if (!agentCaller) {
		console.log(
			`  ${dim("If you are an AI agent, identify yourself: `memb identify <your-name>` (e.g. claude-code, cursor).")}`,
		);
	}
}
...
export async function claimViaOtp(
	...
	if (!code) {
		...
		if (!process.stdin.isTTY) {
			printError(
				...
				`Re-run: memb init --email ${email} --code <code>`,
			);
			process.exit(1);
		}
	...
	if (!verifyResp.ok) {
		...
		if (errCode === "email_already_claimed") {
			console.log(
				`  ${dim("Tip: this email already has a MemB account. Sign in at app.memb.ai with your existing credentials.")}`,
			);
		}
		process.exit(1);
	}
