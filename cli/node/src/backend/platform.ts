...
export class PlatformBackend implements Backend {
	...
	constructor(config: PlatformConfig) {
		...
		this.headers = {
			...
			"X-MemB-Client-Version": CLI_VERSION,
		};
	}
	...
	private async _request(
		...
		const headers = {
			...this.headers,
			"X-MemB-Caller-Type": isAgentMode() ? "agent" : "user",
		};
		...
		if (
			...
			notice = (data as Record<string, unknown>).memb_notice as string;
			// biome-ignore lint/performance/noDelete: intentional strip so downstream consumers don't see duplicate notice
			delete (data as Record<string, unknown>).memb_notice;
		} else if (
			Array.isArray(data) &&
			data.length > 0 &&
			typeof data[0] === "object" &&
			data[0] !== null &&
			"memb_notice" in data[0]
		) {
			notice = (data[0] as Record<string, unknown>).memb_notice as string;
			// biome-ignore lint/performance/noDelete: see above.
			delete (data[0] as Record<string, unknown>).memb_notice;
		}
		if (!notice) {
			notice = resp.headers.get("X-MemB-Notice-Message") ?? null;
		}
