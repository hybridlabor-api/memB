...
const configSlice = createSlice({
  ...
  reducers: {
    ...
    setConfigSuccess: (state, action: PayloadAction<{ openmemory?: OpenMemoryConfig; memb: MemBConfig }>) => {
      if (action.payload.openmemory) {
        state.openmemory = action.payload.openmemory;
      }
      state.memb = action.payload.memb;
    ...
    updateLLM: (state, action: PayloadAction<LLMProvider>) => {
      state.memb.llm = action.payload;
    },
    updateEmbedder: (state, action: PayloadAction<EmbedderProvider>) => {
      state.memb.embedder = action.payload;
    },
    updateMemBConfig: (state, action: PayloadAction<MemBConfig>) => {
      state.memb = action.payload;
    },
  },
});
