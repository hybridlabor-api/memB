...
interface UseConfigApiReturn {
  ...
  saveConfig: (config: { openmemory?: OpenMemoryConfig; memb: MemBConfig }) => Promise<void>;
...
export const useConfig = (): UseConfigApiReturn => {
  ...
  const saveConfig = async (config: { openmemory?: OpenMemoryConfig; memb: MemBConfig }) => {
    setIsLoading(true);
    setError(null);
  ...
  const saveLLMConfig = async (llmConfig: LLMProvider) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/v1/config/memb/llm`, llmConfig);
  ...
  const saveEmbedderConfig = async (embedderConfig: EmbedderProvider) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/v1/config/memb/embedder`, embedderConfig);
