...
> **⚠️ Sunsetting Notice:** OpenMemory is being sunset. For local self-hosted memory with a dashboard, please use the [MemB self-hosted server](https://docs.memb.ai/open-source/overview) instead. Get started with `cd server && make bootstrap`. See the [self-hosted docs](https://docs.memb.ai/open-source/setup) for configuration details.
...
```bash
curl -sL https://raw.githubusercontent.com/membai/memb/main/openmemory/run.sh | bash
```
...
```bash
curl -sL https://raw.githubusercontent.com/membai/memb/main/openmemory/run.sh | OPENAI_API_KEY=your_api_key bash
```
