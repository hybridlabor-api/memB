# Contributing to MemB

First off, thank you for taking the time to contribute! 🎉 MemB is a
...
MemB is a polyglot monorepo, and this guide covers contributing to both the
...
- Search [existing issues](https://github.com/membai/memb/issues) first to see if
  your bug or idea already exists.
- If it doesn't, open a
  [bug report](https://github.com/membai/memb/issues/new?template=bug_report.yml) or
  [feature request](https://github.com/membai/memb/issues/new?template=feature_request.yml).
...
| --------------------- | ---------- | ------------ | --------------- |
| Python SDK (`membai`) | `memb/`    | Python 3.9+  | `hatch`         |
| TypeScript SDK (`membai`) | `memb-ts/` | TypeScript | `pnpm`        |
...
### Contributing to the Python SDK (`memb/`)
...
See the full [Development guide](https://docs.memb.ai/contributing/development) for
environment details.

### Contributing to the TypeScript SDK (`memb-ts/`)
...
|---------|----------|------------|---------|
| `membai` (Python SDK) | PyPI | `v*` | `v0.1.31` |
| `memb-cli` (Python CLI) | PyPI | `cli-v*` | `cli-v0.2.1` |
| `membai` (TypeScript SDK) | npm | `ts-v*` | `ts-v2.4.6` |
| `@memb/cli` (Node CLI) | npm | `cli-node-v*` | `cli-node-v0.1.2` |
| `@memb/vercel-ai-provider` | npm | `vercel-ai-v*` | `vercel-ai-v2.0.6` |
| `@memb/openclaw-memb` | npm | `openclaw-v*` | `openclaw-v1.0.1` |

### How to Release

1. Bump the version in `pyproject.toml` (Python) or `package.json` (Node)
2. Create a [GitHub Release](https://github.com/membai/memb/releases/new) with the matching tag prefix
3. The correct workflow will trigger automatically — verify in the [Actions tab](https://github.com/membai/memb/actions)
