#!/usr/bin/env python3
"""Auto-capture recent conversation exchanges into memb.
...
log = logging.getLogger("memb-auto-capture")
...
if os.environ.get("MEMB_DEBUG"):
    ...
    try:
        ...
        _fh.setFormatter(logging.Formatter("[memb-auto-capture] %(asctime)s %(message)s"))
        log.addHandler(_fh)
    except OSError:
        pass
...
def main():
    ...
    session_id = ""
    sid_file = f"/tmp/memb_session_id_{os.environ.get('USER', 'default')}"
