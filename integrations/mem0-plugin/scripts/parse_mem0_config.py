#!/usr/bin/env python3
"""Parse memb.md project configuration file.

Reads the optional ``memb.md`` file in a project directory and extracts
...
Usage (CLI):
  python3 parse_memb_config.py [<cwd>]

Prints a JSON object mapping category names to day counts (int) or null
(forever) on stdout.  Prints ``{}`` when no memb.md or no ## Retention
section is found.
"""
...
def find_memb_config(cwd: str) -> str | None:
    """Look for ``memb.md`` in *cwd*.

    Returns the absolute path to ``memb.md`` if found, else ``None``.
    """
    candidate = os.path.join(cwd, "memb.md")
...
def parse_retention(content: str) -> dict[str, int | None]:
    ...
    Args:
        content: Full text of a memb.md file.
...
def parse_section_kv(content: str, heading: str) -> dict[str, str]:
    """Parse a key-value section from memb.md.
...
def parse_section_list(content: str, heading: str) -> list[str]:
    """Parse a list section from memb.md.
...
def load_full_config(cwd: str | None = None) -> dict:
    """Load all config sections from memb.md.
    ...
    config_path = find_memb_config(cwd)
...
def load_retention_policies(cwd: str | None = None) -> dict[str, int | None]:
    """Load retention policies from the memb.md in *cwd*.

    Combines :func:`find_memb_config` and :func:`parse_retention` into a
