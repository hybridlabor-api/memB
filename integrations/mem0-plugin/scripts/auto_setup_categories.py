#!/usr/bin/env python3
"""Auto-configure memb's coding-category taxonomy in the background.

Runs from the SessionStart hook (startup only), exactly like auto_import.py.
memb auto-tags every memory with `categories`; by default that list is
...
Design (mirrors auto_import.py):
  ...
  - Gate on a state file (``~/.memb/categories_setup.json``) keyed by a hash of
    the API key -> a hash of the taxonomy. Categories are scoped to the memb
...
# onto sys.path (its module-level bootstrap), so ``from memb import MemoryClient``
...
log = logging.getLogger("memb-auto-categories")
...
if os.environ.get("MEM0_DEBUG"):
    ...
    try:
        ...
        _file_handler.setFormatter(logging.Formatter("[memb-auto-categories] %(asctime)s %(message)s"))
        log.addHandler(_file_handler)
    except OSError:
        pass

STATE_FILE = os.path.expanduser("~/.memb/categories_setup.json")
LOCK_FILE = os.path.expanduser("~/.memb/categories_setup.lock")
...
def main() -> None:
    ...
    try:
        ...
        log.debug("membai SDK not ready yet (venv installing?); will retry next session")
        return
