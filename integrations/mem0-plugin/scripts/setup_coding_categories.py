#!/usr/bin/env python3
"""Replace memb's default category taxonomy with one tuned for coding workflows.

memb auto-tags every memory with one or more `categories`. By default the list
...
Uses the membai SDK (client.project.update). The SDK is installed into a
...
_data_dir = os.environ.get("CLAUDE_PLUGIN_DATA", os.path.join(os.path.expanduser("~"), ".memb", "plugin-data"))
...
def _print_categories(label: str, cats):
    ...
    if cats:
        ...
        print("(none / using memb defaults)")
    print()
...
def main() -> int:
    ...
    try:
        ...
        print(
            "ERROR: membai SDK not found. The plugin's ensure_deps.sh hook should\n"
            "install it automatically on session start. Try restarting Claude Code,\n"
            "or run manually:  pip install membai",
            file=sys.stderr,
        )
        return 1
