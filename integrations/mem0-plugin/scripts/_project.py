...
Resolution priority (project_id):
  ...
  3. Git remote slug: strip protocol/prefix, strip .git, replace / and : with -
     e.g. git@github.com:membai/memb.git -> membai-memb
...
def save_project_mapping(cwd: str, project_id: str) -> None:
    """Write cwd -> project_id (and remote hash key -> project_id) into ~/.memb/project_map.json."""
    memb_dir = os.path.expanduser("~/.memb")
    os.makedirs(memb_dir, exist_ok=True)
    map_path = os.path.join(memb_dir, "project_map.json")
