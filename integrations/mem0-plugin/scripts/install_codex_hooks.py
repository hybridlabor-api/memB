#!/usr/bin/env python3
"""Install MemB lifecycle hooks into ~/.codex/hooks.json.
...
Re-running is idempotent: existing MemB entries (identified by the plugin
...
Usage:
  ...
  python3 install_codex_hooks.py --uninstall   # remove MemB entries
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install or remove MemB Codex hooks.")
    parser.add_argument(
        "--uninstall",
        action="store_true",
        help="Remove MemB entries from ~/.codex/hooks.json and exit.",
    )
    ...
    if args.uninstall:
        ...
        write_config(config)
        print(f"Removed MemB hooks from {HOOKS_FILE}")
        return 0
    ...
    if platform.system() == "Windows":
        print(
            ...
            "installer from WSL or Git Bash, or use MemB via MCP / Direct tools\n",
            file=sys.stderr,
        )
        return 2
    ...
    print(f"Installed MemB hooks into {HOOKS_FILE}")
