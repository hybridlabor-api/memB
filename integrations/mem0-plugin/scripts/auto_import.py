#!/usr/bin/env python3
"""Auto-import declarative project files into memb.

Runs in the background from the SessionStart hook (startup only).
Imports CLAUDE.md, AGENTS.md, .cursorrules, .windsurfrules, memb.md
...
log = logging.getLogger("memb-auto-import")
...
if os.environ.get("MEMB_DEBUG"):
    ...
    try:
        ...
        _file_handler.setFormatter(logging.Formatter("[memb-auto-import] %(asctime)s %(message)s"))
...
TARGET_FILES = ["CLAUDE.md", "AGENTS.md", ".cursorrules", ".windsurfrules", "memb.md"]
HASH_STORE = os.path.expanduser("~/.memb/file_hashes.json")
LOCK_FILE = os.path.expanduser("~/.memb/auto_import.lock")
...
def save_hashes(hashes: dict[str, str]) -> None:
    ...
    memb_dir = os.path.expanduser("~/.memb")
...
def post_memory(api_key: str, content: str, user_id: str, filename: str, project_id: str, branch: str = "") -> bool:
    """POST a project profile memory to the MemB REST API."""
    metadata = {
    ...
    for filename in TARGET_FILES:
        ...
        if hashes.get(hash_key) == current_hash:
            if already_imported(api_key, user_id, project_id, filename):
                log.debug("Unchanged and still in memb, skipping: %s", filename)
                continue
            ...
            log.debug("Already in memb, updating hash store: %s", filename)
