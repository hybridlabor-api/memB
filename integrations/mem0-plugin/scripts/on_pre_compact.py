#!/usr/bin/env python3
"""Capture session state via the MemB REST API.

Safety net for PreCompact and Stop hooks — reads the transcript JSONL,
extracts structured session state, and stores it in MemB directly.
...
log = logging.getLogger("memb-capture")
log.setLevel(logging.DEBUG)
_handler = logging.StreamHandler(sys.stderr)
_handler.setFormatter(logging.Formatter("[memb-capture] %(message)s"))
...
if os.environ.get("MEMB_DEBUG"):
    ...
    try:
        ...
        _file_handler.setFormatter(logging.Formatter("[memb-capture] %(asctime)s %(message)s"))
        log.addHandler(_file_handler)
    except OSError:
        pass

API_URL = "https://api.memb.ai"
MAX_TAIL_LINES = 500
...
def build_content(state: dict, source: str) -> str:
    ...
    Keep it short — memb infer=True will extract structured facts.
    """
    parts = []
...
def store_memory(api_key: str, content: str, user_id: str, source: str, session_id: str = "", project_id: str = "", branch: str = "") -> bool:
    """Store session state as a memory via the MemB REST API."""
...
def format_status(state: dict, source: str, stored: bool, skipped_reason: str = "") -> str:
    ...
    if skipped_reason:
        return f"{icon} MemB {label} — {activity} — {skipped_reason}"
    elif stored:
        return f"{icon} MemB {label} — {activity} — saved to memb"
    else:
        return f"{icon} MemB {label} — {activity} — nothing to capture"
...
def main():
    ...
    if not api_key:
        log.debug("MEMB_API_KEY not set, skipping capture")
        if show_status:
            print("✨ MemB — no API key, skipping capture")
        return
    ...
    stats_file = f"/tmp/memb_session_stats_{os.environ.get('USER', 'default')}.json"
    try:
