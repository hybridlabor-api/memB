...
files touched so far, then stores via memb API with infer=True. Uses
...
log = logging.getLogger("memb-session-summary")
...
if os.environ.get("MEM0_DEBUG"):
    ...
    try:
        ...
        _fh.setFormatter(logging.Formatter("[memb-session-summary] %(asctime)s %(message)s"))
        log.addHandler(_fh)
    except OSError:
        pass
...
def build_summary_prompt(assistant_msg: str, files: list[str]) -> str:
    """Build a structured prompt that helps memb's AI extract a good summary."""
    files_section = ""
    if files:
