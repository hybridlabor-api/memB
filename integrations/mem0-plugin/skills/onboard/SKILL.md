---
name: onboard
description: Sets up memb for a new project including API key configuration, MCP authentication, project file import, and coding categories. Use on first run in a new project, when API key needs updating, or to re-run initial setup after configuration changes.
---

# MemB Onboarding Wizard

Run this wizard to set up the memb plugin for the current project. Complete in ~60 seconds.
...
## Step 0: Ensure membai SDK is installed

The plugin installs the `membai` Python SDK automatically on session start via a venv in `${CLAUDE_PLUGIN_DATA}/venv`. If Step 5 (categories) fails with an import error, run:
...
- API key not found. Let's set it up.
  ...
  1. Get your API key from https://app.memb.ai/dashboard/api-keys
...
First, check if MCP tools are already available using ToolSearch with query `"memb search_memories"`. The exact tool name varies by install method (may be `mcp__memb__search_memories` or `mcp__plugin_memb_memb__search_memories`).
...
3. Check the MCP server is listed: run `/mcp` and look for `mcp.memb.ai`
4. If the server shows an error, ask the user to restart Claude Code and run `/memb:onboard` again
5. If all checks pass but tools are still missing: "Restart Claude Code and run `/memb:onboard` again."
...
Project files (CLAUDE.md, AGENTS.md, etc.) are automatically imported into memb when a session starts. This step verifies import status and triggers a re-import if needed.

### 4a: Detect project files

```bash
for f in CLAUDE.md AGENTS.md .cursorrules .windsurfrules memb.md; do
...
- If output contains `Imported` lines:
  ```
  - Importing project files into memb... done.
    <N> file(s) imported (<M> chunks). These are stored verbatim for future context.
  ```
- If output contains only `skipping` lines:
  ```
  - Project files already in memb (imported during session start). Verified server-side.
  ```
- If output contains `re-importing`:
  ```
  - Project files were missing from memb. Re-imported successfully.
  ```
- If output contains errors or no files were processed:
  ```
  - Project file import failed. Check API key and retry with: /memb:onboard
  ```
...
Memory is now active for this project. Start working — memb will
automatically search relevant context and capture learnings.

Run /memb:tour to see what memb already knows about this project.
```
