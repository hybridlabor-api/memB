...
- `/memb:switch-project <project-name>` — switch to a specific project scope
- `/memb:switch-project --global` — enable global search (all memories, all users, all projects)
- `/memb:switch-project --no-global` — disable global search and return to per-project scoping

## Execution

### If `--global` flag is provided:

1. Set `global_search: true` in `~/.memb/settings.json` using the Bash tool:

   ```bash
   python3 -c "
   import json, os
   settings_file = os.path.expanduser('~/.memb/settings.json')
   settings = {}
...
1. Set `global_search: false` in `~/.memb/settings.json` using the Bash tool:
...
2. Write the mapping to `~/.memb/project_map.json` using the Bash tool:

   ```bash
   python3 -c "
   import json, os
   map_file = os.path.expanduser('~/.memb/project_map.json')
   mapping = {}
