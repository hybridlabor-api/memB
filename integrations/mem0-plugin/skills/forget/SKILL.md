...
- A search query: `/memb:forget auth module decisions`
...
If `recent_ids` is empty, tell the user: "No recent memory IDs tracked this session. Try `/memb:tour` to browse recent memories, or `/memb:forget <search query>` to find specific ones."
