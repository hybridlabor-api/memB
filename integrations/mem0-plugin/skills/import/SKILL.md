...
Import memories from a memb export file into the current project.

## Execution

### Step 1: Determine the export file to import

If the user provided a filename as an argument to `/memb:import <filename>`, use that file.

Otherwise, list `.md` files in the current directory whose names contain `memb-export`:

```bash
ls -1 *.md 2>/dev/null | grep memb-export || echo "No export files found"
```

If multiple files are found, ask the user which one to import. If none are found, print:
```
No memb-export files found in the current directory.
Run /memb:export first, or provide the filename: /memb:import <path-to-file>
```
...
When invoked with `--tools` (e.g., `/memb:import --tools`), detect and import
