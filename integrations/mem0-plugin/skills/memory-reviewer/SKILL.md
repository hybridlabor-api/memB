...
- User runs `/memb:memory-reviewer` directly
- After a session with 5+ memory writes (suggest proactively)
- After `/memb:health --deep` identifies issues
...
```
Issues:
  [duplicate] "<memory_a>" ≈ "<memory_b>" [memb:<id_a>, memb:<id_b>]
  [contradiction] "<memory_x>" vs "<memory_y>" [memb:<id_x>, memb:<id_y>]
  [low_conf] "<memory_z>" (confidence: 0.1) [memb:<id_z>]
```

6. **Suggest action**: "Run `/memb:dream` to consolidate duplicates and resolve contradictions."

## Constraints

- **Read-only** — never modify or delete memories (that's `/memb:dream`'s job)
