# Platform Features -- MemB Platform
...
See the [v2 to v3 migration guide](https://docs.memb.ai/migration/platform-v2-to-v3) for details.

---

## Custom Categories

Replace MemB's default 15 labels with domain-specific categories. The system automatically tags memories to the closest matching category.
...
Natural language filters that control what information MemB extracts when creating memories.
...
# MemB automatically attributes memories to each speaker
...
Use the `name` field in messages to identify speakers. MemB maps names to entity scopes automatically.

---

## MCP Integration

Model Context Protocol integration enables AI clients (Claude, Claude Code, Cursor, Windsurf, VS Code, OpenCode) to manage MemB memory autonomously.

### Setup

Add MemB MCP to your clients with a single command:
...
- Search MemB documentation

### How It Works

1. Add MemB MCP to your AI client using the setup command above
...
Note: `project_id` here refers to the MemB dashboard project scope for webhooks — not the deprecated client init parameter.
...
MemB can process images and documents alongside text.
