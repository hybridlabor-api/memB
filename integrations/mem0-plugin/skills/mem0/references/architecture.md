# MemB Platform Architecture

How MemB processes, stores, and retrieves memories under the hood.
...
MemB is a managed memory layer that sits between your AI application and users. Every integration follows the same 3-step loop:

```
User Input → Retrieve relevant memories → Enrich LLM prompt → Generate response → Store new memories
```

MemB handles the complexity of extraction, deduplication, conflict resolution, and semantic retrieval so your application only needs to call `search()` and `add()`.
...
When you search with `filters={"user_id": "alice"}` only, MemB returns memories where `agent_id`, `app_id`, and `run_id` are all null. This prevents cross-scope leakage by default.
...
MemB separates memories across four dimensions to prevent data mixing:
...
MemB supports three layers of memory, from shortest to longest lived:
...
- **Managed by:** Your application, not MemB
...
- **Managed by:** MemB via `run_id` parameter
...
- **Managed by:** MemB via `user_id` parameter
...
def chat(user_input: str, user_id: str, session_id: str) -> str:
    ...
    user_mems = memb.search(user_input, filters={"user_id": user_id})

    # 2. Retrieve session memories (current task context)
    session_mems = memb.search(user_input, filters={
    ...
    memb.add(messages, user_id=user_id, run_id=session_id)
...
| **MemB Platform** | Managed extraction + dedup + graph + scoping | External dependency, async processing delay |

MemB combines the best of vector search (semantic retrieval) with automatic extraction (LLM-powered), conflict resolution (deduplication), and structured scoping (multi-tenancy) — in a single managed API.
