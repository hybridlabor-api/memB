---
name: health
description: Diagnoses memb connectivity, API key validity, and memory read/write functionality. Use when memory operations fail, searches return empty, add_memory errors occur, MCP connection drops, or to verify the plugin is working correctly.
---
...
```bash
STATS_FILE="/tmp/memb_session_stats_${USER}.json"
...
When invoked with `--deep` (e.g., `/memb:health --deep`), run the standard 5 checks above **plus** a memory quality scan.
