"""Tests for parse_memb_config.py — memb.md retention policy parser."""
...
def test_parse_retention_valid_section():
    ...
    from parse_memb_config import parse_retention
...
def test_load_retention_policies_with_tmp_file(tmp_path):
    """load_retention_policies reads a real memb.md from disk."""
    from parse_memb_config import load_retention_policies

    memb_md = tmp_path / "memb.md"
    memb_md.write_text(
        """\
# My Project
...
def test_load_retention_policies_no_memb_md_returns_empty(tmp_path):
    """load_retention_policies returns {} when no memb.md exists."""
    from parse_memb_config import load_retention_policies
...
def test_load_retention_policies_no_retention_section_returns_empty(tmp_path):
    """load_retention_policies returns {} when memb.md has no ## Retention."""
    from parse_memb_config import load_retention_policies
...
def test_cli_main_prints_json(tmp_path):
    """CLI: python parse_memb_config.py <cwd> prints valid JSON."""
    memb_md = tmp_path / "memb.md"
    memb_md.write_text(
        "## Retention\nsession_state: 90d\nuser_preference: forever\n",
        encoding="utf-8",
    ...
    result = subprocess.run(
        [sys.executable, os.path.join(SCRIPTS_DIR, "parse_memb_config.py"), str(tmp_path)],
        capture_output=True,
        text=True,
    )
...
def test_cli_main_no_file_prints_empty_json(tmp_path):
    """CLI: prints '{}' when no memb.md exists."""
    result = subprocess.run(
        [sys.executable, os.path.join(SCRIPTS_DIR, "parse_memb_config.py"), str(tmp_path)],
        capture_output=True,
        text=True,
...
def test_parse_section_kv_basic():
    ...
    from parse_memb_config import parse_section_kv
...
"""
    ...
    assert result == {"user_id": "kartik", "project_id": "memb"}
...
def test_parse_section_list_basic():
    ...
    from parse_memb_config import parse_section_list
...
def test_load_full_config_all_sections(tmp_path):
    """load_full_config extracts all sections from memb.md."""
    from parse_memb_config import load_full_config
...
def test_load_full_config_no_file(tmp_path):
    """load_full_config returns {} when no memb.md exists."""
    from parse_memb_config import load_full_config
...
def test_cli_full_flag(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, os.path.join(SCRIPTS_DIR, "parse_memb_config.py"), "--full", str(tmp_path)],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
