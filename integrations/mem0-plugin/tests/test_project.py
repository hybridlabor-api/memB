...
def test_remote_url_to_slug_various_formats():
    ...
    assert _remote_url_to_slug("https://github.com/membai/memb.git") == "membai-memb"
    assert _remote_url_to_slug("git@github.com:membai/memb.git") == "membai-memb"
...
def test_resolve_project_id_priority_order(tmp_git_repo, monkeypatch):
    ...
    # Git remote gives "membai-memb"
    assert resolve_project_id(str(tmp_git_repo)) == "membai-memb"
...
def test_resolve_project_id_remote_hash_fallback(tmp_git_repo, tmp_path):
    ...
    _sp.run(
        ["git", "remote", "add", "origin", "https://github.com/membai/memb.git"],
        cwd=new_repo,
        capture_output=True,
        check=True,
    )
