/**
 * Auto-dream: gated automatic memory consolidation for the MemB OpenCode plugin.
 *
...
function statePath(stateDir: string): string {
  return join(stateDir, "memb-dream-state.json");
}

function lockPath(stateDir: string): string {
  return join(stateDir, "memb-dream.lock");
}
...
You are running memory consolidation. Complete these steps using the memb memory tools (get_memories, add_memory, delete_memory):
