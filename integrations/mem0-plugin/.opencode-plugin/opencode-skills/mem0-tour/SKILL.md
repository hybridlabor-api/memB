...
Show the user what memb has stored for the current project.

## Cross-project mode

When invoked with `--all-projects` (e.g., `/memb-tour --all-projects` or
`/memb-tour --all-projects auth middleware`), search across ALL projects:
...
When `/memb-tour` receives a search query argument (e.g., `/memb-tour auth middleware`)
...
1. **Platform `categories` field** (array on each memory, auto-assigned by MemB). Use the first category value.
...
tooling_setup (119)
  ...
  3. Assistant determined that the symlink method for loading the MemB plugin was failing...
  ... and 116 more

bug_fixes (78)
  1. Fixed getAll filter format from flat object to AND-wrapped array for membai TS SDK v3...
...
Start working - memb captures learnings automatically, or use /memb-remember to save something now.
```
