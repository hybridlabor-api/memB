...
# MemB Dream — Memory Consolidation
...
1. Look for `.memb.json` first. If it exists, parse it as JSON and read the
   `retention` field (a dict of `category → days | null`).
2. If `.memb.json` is not present, look for `.memb.md`. If it exists, scan it
...
Merges (<N>):
  [memb:<id1>] + [memb:<id2>] → "<merged content, 100 chars>"

Conflicts (<N>):
  [memb:<idA>] vs [memb:<idB>] — "<topic>" [A/B/skip]

Prune (<N>):
  [memb:<id>] — <type>, <age>d old
...
3. `add_memory` with:
   ...
   - `metadata={"type": "<original type>", "branch": "<active_branch>", "confidence": <higher of the two original scores>, "source": "memb-dream"}`
   - `infer=False`
...
When invoked with `--auto` (e.g., `/memb-dream --auto`), run non-interactively:
...
Before doing any work, check for a lock file at `/tmp/memb_dream_auto.lock`:
- If the lock file exists and is less than 10 minutes old, print `[memb-dream --auto] Another run in progress — skipping.` and stop.
...
3. Print a compact summary:
   ```
   [memb-dream --auto] project=<id>  merged=<N>  pruned=<N>  conflicts_skipped=<N>
   ```
4. If contradictions were detected but skipped, check if a `memb-dream-auto` reminder already exists before storing one:
   - Search for existing reminders: `search_memories(query="memb-dream contradictions manual review", filters={"AND": [{"user_id": "<active_user_id>"}, {"app_id": "<active_project_id>"}, {"metadata": {"source": "memb-dream-auto"}}]}, top_k=1)`
   - If a result exists with similarity > 0.9, skip storing the reminder (one already exists).
   - If no match, store the reminder:
   ```python
   add_memory(
       text="memb-dream detected <N> contradiction(s) requiring manual review. Run /memb-dream to resolve them interactively.",
       user_id="<active_user_id>",
       app_id="<active_project_id>",
       metadata={"type": "task_learning", "source": "memb-dream-auto", "branch": "<active_branch>"},
       infer=False,
   )
   ```
...
- `/memb-forget` — targeted deletion of specific memories (search + confirm + delete)
- `/memb-status --deep` — quick quality scan without applying changes
