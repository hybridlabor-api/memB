...
no explicit `scope` is given. The setting persists in `~/.memb/settings.json`
...
2. Read the existing settings so you preserve every other key. Use the Read tool
   on `~/.memb/settings.json` (it may not exist yet — treat a missing file as
   `{}`).
