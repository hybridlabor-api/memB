/**
 * Plugin telemetry for the MemB OpenCode plugin — anonymous usage tracking
 * via PostHog.
 *
 * Emits the SAME event schema as the MemB editor plugin's telemetry.py
...
function _loadPluginVersion(): string {
  ...
  for (const rel of ["./package.json", "../package.json"]) {
    try {
      ...
      if (pkg?.name === "@memb/opencode-plugin" && pkg.version) return pkg.version;
    } catch {
      /* try next candidate */
    }
  }
...
function distinctId(apiKey: string): string {
  ...
  // PostHog whether they use OpenCode or any other MemB editor plugin.
