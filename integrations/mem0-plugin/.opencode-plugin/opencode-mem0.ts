// MemB memory plugin for OpenCode: captures and recalls memories across sessions
// (add / search / manage) via the MemB platform, wired through OpenCode plugin hooks.
// Memory operations are exposed as native OpenCode tools backed by the membai SDK
...
/** Read & parse `~/.memb/settings.json`, returning {} when missing/invalid. */
function loadSettings(): Record<string, unknown> {
  try {
    const settingsPath = join(homedir(), ".memb", "settings.json");
...
/**
 * The user's persisted default memory scope (set via the `memb-scope` skill).
...
async function autoSetupCategories(memb: MemoryClient, apiKey: string): Promise<void> {
  const stateDir = join(homedir(), ".memb");
  ...
  try {
    const project = await memb.getProject({fields: ["customCategories"]});
    ...
    await memb.updateProject({customCategories: CODING_CATEGORIES as any});
...
const MemBPlugin: Plugin = async (ctx) => {
  ...
  const memb = new MemoryClient({apiKey});
  ...
  const membStateDir = join(homedir(), ".memb");
  const dreamConfig = loadDreamConfig(membStateDir);
  ...
  const emitSessionStop = () => {
    ...
    if (dreamTriggered) {
      if (dreamWriteSeen) {
        recordDreamCompletion(membStateDir);
        captureEvent("dream_completed", {}, apiKey, appId);
      }
      releaseDreamLock(membStateDir);
  ...
  Promise.resolve().then(() => autoSetupCategories(memb, apiKey)).catch(() => {
  });

  // Register a `/memb-<skill>` slash command per bundled skill. OpenCode's TUI
  ...
  // slash commands), so this is what makes `/memb-scope` etc. typeable.
  function registerCommands(skillsDir: string, opencodeConfig: any) {
    for (const entry of readdirSync(skillsDir, {withFileTypes: true})) {
      ...
      let desc = `MemB ${entry.name} skill`;
      try {
...
Use the memb memory tools (add_memory, search_memories, get_memories, get_memory, update_memory, delete_memory, delete_all_memories, delete_entities, list_entities, get_event_status) as instructed by the skill.
...
- branch: ${branch}`,
  ...
  // user's persisted default scope (read fresh so /memb-scope applies at once).
  ...
  return {
    ...
    config: async (opencodeConfig: any) => {
      ...
      // Register the /memb-* slash commands (the TUI slash menu reads these from
    ...
    tool: {
      ...
      search_memories: tool({
        ...
        async execute(args) {
          ...
          const res = await memb.search(args.query, {
            filters,
            topK,
          });
          return JSON.stringify(res);
      ...
      get_memory: tool({
        ...
        async execute(args) {
          ...
          const res = await memb.get(args.id);
          return JSON.stringify(res);
        }
      }),
      ...
      update_memory: tool({
        ...
        async execute(args) {
          ...
          const res = await memb.update(args.id, {
            text: args.text,
      ...
      delete_memory: tool({
        ...
        async execute(args) {
          ...
          const res = await memb.delete(args.id);
          return JSON.stringify(res);
        }
      }),
      ...
      delete_all_memories: tool({
        ...
        async execute(args) {
          ...
          const res = await memb.deleteAll({
      ...
      delete_entities: tool({
        ...
        async execute(args) {
          ...
          const res = await memb.deleteUsers({
            userId: args.user_id,
      ...
      get_event_status: tool({
        ...
        async execute(args) {
          ...
          const response = await memb.client.get(`/v1/event/${args.event_id}/`);
  ...
  async function chatMessageHook(input: any, output: any) {
    ...
    if (!initialized) {
      ...
      if (dreamConfig.enabled) {
        incrementSessionCount(membStateDir, sessionId);
      }
      ...
      try {
        ...
        if (memoryCount > 0) {
          systemContext.push(
            "Search memb for recent decisions and task learnings before responding. Run 2 parallel searches: one for decision type, one for task_learning type.",
          );
          try {
            const res = await memb.search(
            ...
            if (memories.length > 0) {
              const memLines = memories
                .map((m) => `- ${m.memory}`)
                .join("\n");
              systemContext.push(`Prior context from memb:\n${memLines}`);
            }
          } catch {
          }
        }
        ...
        systemContext.push(
          "MemB searches apply when user references past work, decision questions, errors, or non-trivial tasks. Queries use noun-phrases, 2-4 parallel calls with different metadata.type filters, and include user_id + app_id.",
        );
        systemContext.push(SCOPE_GUIDANCE);
        const activeScope = loadDefaultScope();
        if (activeScope !== "project") {
          systemContext.push(
            `Active default memory scope is "${activeScope}" (set via /memb-scope). Memory tools use this when no explicit scope is given: "session" limits to this run (run_id="${sessionId}"); "global" spans all your projects (app_id="*"). Pass an explicit scope to override per call. delete_all_memories still requires an explicit scope="global" to delete user-wide.`,
          );
        }
      } catch (err: any) {
        try {
      ...
      if (dreamConfig.enabled && dreamConfig.auto && !dreamTriggered) {
        const gates = checkCheapGates(membStateDir, dreamConfig);
        const memGate = checkMemoryGate(memoryCount, dreamConfig);
        if (gates.proceed && memGate.pass && acquireDreamLock(membStateDir)) {
          dreamTriggered = true;
          ...
          if (waiting) {
            try {
              await client.app.log({
                body: {service: "memb", level: "info", message: `auto-dream waiting — ${waiting}`},
              });
            } catch {
            }
          }
    ...
    if (hasResume) {
      try {
        ...
        const [stateRes, decisionsRes] = await Promise.all([
          memb.search("session state current task", {
            filters: resumeFilters,
            topK: 3,
          }),
          memb.search("recent decisions and learnings", {
            filters: resumeFilters,
            topK: 3,
          }),
        ]);
    ...
    if (!hasResume && memoryCount > 0) {
      try {
        const msgFilters = globalSearch
          ? {OR: [{user_id: "*"}]}
          : {AND: [{user_id: userId}, {app_id: appId}]};
        const res = await memb.search(safeText, {
          filters: msgFilters,
          topK: 5,
        });
        stats.searches++;
    ...
    if (msgCount % 3 === 0) {
      Promise.resolve().then(async () => {
        try {
          await memb.add([{role: "user", content: safeText}], {
            user_id: userId,
            app_id: appId,
            metadata: {
              type: "auto_capture",
  ...
  async function chatMessagesTransformHook(_input: any, output: { messages: { info: any; parts: any[] }[] }) {
    ...
    const marker = "## MemB Memory Context";
  ...
  async function toolExecuteAfterHook(input: any, _output: any) {
    ...
    if (toolName === "bash" && toolOutput.length >= 50) {
      ...
      try {
        ...
        const res = await memb.search(`error: ${errorQuery}`, {
  ...
  async function compactionHook(input: { sessionID?: string }, output: { context: string[]; prompt?: string }) {
    try {
      ...
      Promise.resolve().then(async () => {
        try {
          await memb.add([{role: "user", content: summaryContent}], {
            user_id: userId,
            app_id: appId,
            metadata: {
              type: "session_state",
      ...
      const compactFilters = globalSearch
        ? {OR: [{user_id: "*"}]}
        : {AND: [{user_id: userId}, {app_id: appId}]};
      const res = await memb.search("session state decisions learnings", {
