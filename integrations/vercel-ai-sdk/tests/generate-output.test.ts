...
describe.each(testConfig.providers)('TESTS: Generate/Stream Text with model %s', (provider: Provider) => {
  ...
  let memb: ReturnType<typeof testConfig.createTestClient>;
  jest.setTimeout(50000);
  
  beforeEach(() => {
    memb = testConfig.createTestClient(provider);
  });
  ...
  it("should generate text using memb model", async () => {
    const { text } = await generateText({
      model: memb(provider.activeModel, {
        user_id: userId,
      }),
  ...
  it("should stream text using MemB provider with new streaming approach", async () => {
    // Create the base model
    const baseModel = memb(provider.activeModel, {
      user_id: userId,
    });
