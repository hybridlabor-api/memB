# MemB AI SDK Provider

The **MemB AI SDK Provider** is a community-maintained library developed by [MemB](https://memb.ai/) to integrate with the Vercel AI SDK. This library brings enhanced AI interaction capabilities to your applications by introducing persistent memory functionality. With MemB, language model conversations gain memory, enabling more contextualized and personalized responses based on past interactions.

Discover more of **MemB** on [GitHub](https://github.com/membai).
Explore the [MemB Documentation](https://docs.memb.ai/overview) to gain deeper control and flexibility in managing your memories.
...
```bash
npm install @memb/vercel-ai-provider
```
...
1. Obtain your [MemB API Key](https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=vercel-ai-sdk-readme) from the MemB dashboard.

2. Initialize the MemB Client:
...
import { addMemories } from "@memb/vercel-ai-provider";
...
These memories are now stored in your profile. You can view and manage them on the [MemB Dashboard](https://app.memb.ai/dashboard/users?utm_source=oss&utm_medium=vercel-ai-sdk-readme).
...
```typescript
await addMemories(messages, { user_id: "borat", membApiKey: "m0-xxx", org_id: "org_xx", project_id: "proj_xx" });
await retrieveMemories(prompt, { user_id: "borat", membApiKey: "m0-xxx", org_id: "org_xx", project_id: "proj_xx" });
await getMemories(prompt, { user_id: "borat", membApiKey: "m0-xxx", org_id: "org_xx", project_id: "proj_xx" });
```
...
import { retrieveMemories } from "@memb/vercel-ai-provider";
...
- `createMemB()`: Initializes a new memb provider instance with optional configuration
...
We also have support for `agent_id`, `app_id`, and `run_id`. Refer [Docs](https://docs.memb.ai/api-reference/memory/add-memories).
