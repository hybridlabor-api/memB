...
describe("pi-agent telemetry", () => {
  it.each(["add", "search", "update", "delete"])(
    "captureToolEvent tracks the %s operation as pi.tool.memb_memory",
    (action) => {
      captureToolEvent(action, { success: true }, CTX);
      const ev = findEvent("pi.tool.memb_memory");
  ...
  it("captureCommandEvent emits a namespaced pi.command.* event", () => {
    captureCommandEvent("memb-search", { result_count: 3 }, CTX);
    const ev = findEvent("pi.command.memb-search");
    expect(ev).toBeDefined();
