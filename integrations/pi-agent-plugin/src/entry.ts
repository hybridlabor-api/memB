...
export async function buildRecallContext(
  ...
  try {
    ...
    return `<memb-relevant-memories>\nRetrieved automatically for the current request. This is a shallow first pass — search memb_memory for more if you need it.\n${formatMemoryList(memories)}\n</memb-relevant-memories>`;
  } catch {
    return "";
  }
}

export default function membExtension(pi: ExtensionAPI): void {
  ...
  const memb = new MemoryClient({ apiKey: config.apiKey });
  ...
  registerMemoryTool(pi, memb, config, getScopeCtx, telemetryCtx);
  registerCommands(pi, memb, config, getScopeCtx, telemetryCtx);
  setupAutoCapture(pi, memb, config, getScopeCtx, telemetryCtx);
  ...
  pi.on("before_agent_start", async (event, _ctx) => {
    ...
    const recall = await buildRecallContext(
      ...
      (q) => memb.search(q, { filters: resolveSearchFilters("project", scopeCtx) }),
    );
    if (recall) extra += "\n\n" + recall;
