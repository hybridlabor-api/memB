...
const TELEMETRY_ID_PATH = path.join(CONFIG_DIR, "memb-telemetry-id.json");
...
function getOrCreateAnonymousId(): string {
  ...
  const newId = `pi-memb-anon-${randomUUID().replace(/-/g, "")}`;
  try {
...
function maybeBuildIdentifyEvent(distinctId: string): Record<string, unknown> | null {
  ...
  if (!distinctId || distinctId.startsWith("pi-memb-anon-")) return null;
  try {
    if (!fs.existsSync(TELEMETRY_ID_PATH)) {
      _identifyDone = true;
      return null;
...
export function captureToolEvent(
  ...
  captureEvent("pi.tool.memb_memory", { action, ...properties }, ctx);
}
