...
import type { Scope, ScopeContext, MemBConfig } from "../types.ts";
  ...
  return async (params: ToolParams, signal?: AbortSignal) => {
    ...
    switch (params.action) {
      case "search": {
        ...
        const result = await memb.search(params.query, { filters });
      ...
      case "update": {
        ...
        const updateResult = await memb.update(params.memory_id, { text: params.content });
      ...
      case "delete": {
        ...
        const result = await memb.delete(params.memory_id);
        return {
      ...
      case "delete_all": {
        ...
        const result = await memb.deleteAll(delParams);
        return {
...
export function registerMemoryTool(
  ...
  pi.registerTool({
    ...
    label: "MemB Memory",
    description:
      "Search, add, update, and manage persistent semantic memories powered by MemB. Memories persist across sessions and devices. Use action \"search\" proactively -- before answering anything that may depend on what the user told you earlier -- and run multiple searches with different phrasings for multi-part questions. Output is truncated to 200 lines / 50KB.",
    promptSnippet: "Semantic memory search and storage via MemB",
    promptGuidelines: [
      'Use memb_memory with action "search" proactively whenever the request may depend on the user\'s past work, preferences, decisions, or environment -- not only when they explicitly mention the past',
      ...
      'Use memb_memory with action "add" to save important facts, preferences, goals, decisions, or lessons the user shares',
      'Use memb_memory with action "update" to modify an existing memory — requires memory_id and content. Preserves the memory ID',
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      const exec = buildToolExecute(memb, scopeCtx, config.defaultScope);
