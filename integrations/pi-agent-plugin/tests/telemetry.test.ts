...
describe("telemetry", () => {
  ...
  it("captureToolEvent uses pi.tool.memb_memory event name with action property", () => {
    captureToolEvent("search", { success: true, latency_ms: 42 });
    const queue = _getEventQueue();
    expect(queue).toHaveLength(1);
    expect(queue[0].event).toBe("pi.tool.memb_memory");
    const props = queue[0].properties as Record<string, unknown>;
  ...
  it("captureCommandEvent uses pi.command.<name> event name", () => {
    captureCommandEvent("memb-search", { result_count: 5 });
    const queue = _getEventQueue();
    expect(queue).toHaveLength(1);
    expect(queue[0].event).toBe("pi.command.memb-search");
    const props = queue[0].properties as Record<string, unknown>;
