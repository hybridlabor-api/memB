...
2. **Run 2-4 parallel searches** using `memb_memory` tool with `action="search"` and different query angles:
...
context-loader: loaded <N> memories for "<task summary>"
  - [decisions] <content> [memb:<short_id>]
  - [preferences] <content> [memb:<short_id>]
  - [lessons] <content> [memb:<short_id>]
```
