...
Use `memb_memory` tool with `action="get_all"` to retrieve every memory.
...
**Merges:** Delete both originals, add the merged version using `memb_memory` with `action="add"`.

**Contradictions (resolved):** Delete the loser using `memb_memory` with `action="delete"`.

**Prunes:** Delete each using `memb_memory` with `action="delete"`.
...
```
[memb-dream --auto] merged=<N> pruned=<N> conflicts_skipped=<N>
```

## See also

- `/memb-forget` — targeted deletion of specific memories
- `/memb-status` — quick health check
