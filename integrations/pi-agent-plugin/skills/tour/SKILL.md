...
Show the user what MemB has stored — a full walkthrough of all memories grouped by category.

## Cross-project mode

When invoked with `--all-projects` (e.g., `/memb-tour --all-projects`), search across ALL projects:

1. Use `memb_memory` tool with `action="get_all"`, `scope="global"` — no project filter.
...
When `/memb-tour` receives a search query argument (e.g., `/memb-tour cooking recipes`), run in **search mode** — compact one-liner results:

1. Use `memb_memory` tool with `action="search"`, `query=<query>`.
...
Use `memb_memory` tool with `action="get_all"`.
...
```
No memories stored yet. Start a conversation — MemB captures learnings automatically, or use /memb-remember to store something manually.
```
