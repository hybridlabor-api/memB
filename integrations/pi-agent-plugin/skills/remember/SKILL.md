...
Store a fact, preference, or learning directly into MemB.

## Execution

### Step 1: Extract the content

The user provides the content as an argument: `/memb-remember <text>`
...
Use the `memb_memory` tool with:
- `action="add"`
- `content="<the user's text>"`

The `/memb-remember` command stores verbatim — no inference. This is already handled by the command.
