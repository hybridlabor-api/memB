...
**If search query:**
- Use `memb_memory` tool with `action="search"`, `query=<query>`.
...
Use `memb_memory` tool with `action="add"`, `content="[PINNED] <original memory text>"`.

Then delete the original using `memb_memory` with `action="delete"` and the original memory ID.

**For new memories** (user wants to pin text that isn't stored yet):
- Use `memb_memory` tool with `action="add"`, `content="[PINNED] <the user's text>"`.
