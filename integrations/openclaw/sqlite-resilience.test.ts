...
import { membConfigSchema, createProvider } from "./index.ts";
...
// ---------------------------------------------------------------------------
describe("membConfigSchema — disableHistory", () => {
  const baseConfig = {
    mode: "open-source",
    oss: {
  ...
  it("omits disableHistory when not provided", () => {
    const cfg = membConfigSchema.parse(baseConfig);
...
describe("OSSProvider — graceful SQLite fallback", () => {
  ...
  it("retries with disableHistory: true when initial construction fails", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining("[memb] Memory initialization failed"),
