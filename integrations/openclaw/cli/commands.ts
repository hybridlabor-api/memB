/**
 * CLI subcommand registration for the OpenClaw MemB plugin.
 *
 * Registers all `openclaw memb <subcommand>` commands:
 *
 * Memory:
 ...
 *
 * Management:
 *   - init        : Authenticate with MemB Platform (email or API key)
 ...
 *
 * Naming conventions match the Python CLI (`memb init`, `memb search`, etc.)
 */
...
/**
 * POST JSON to a MemB API endpoint. Returns parsed body on success, null on failure.
  ...
  try {
    resp = await fetch(url, {
      ...
      headers: {
        ...
        "X-MemB-Source": "OPENCLAW",
        "X-MemB-Client-Language": "node",
      },
...
export function registerCliCommands(
  ...
  api.registerCli(
    ({ program }) => {
      ...
      // ====================================================================
      // init (matches: memb init)
      // ====================================================================

      memb
        .command("init")
        .description("Set up MemB — authenticate and configure")
        ...
        .action(
          async (opts: {
            ...
            try {
              const baseUrl = "https://api.memb.ai";
              const existingAuth = readPluginAuth();
              ...
              if (opts.apiKey) {
                ...
                let message: string;
                if (check.ok) {
                  message = "API key validated. Connected to MemB Platform.";
              ...
              if (opts.email) {
                ...
                if (sent) {
                  const nextCmd = `openclaw memb init --email ${email} --code <CODE>`;
              ...
              if (!process.stdin.isTTY) {
                ...
                console.log("    openclaw memb init --api-key <key>");
                console.log("    openclaw memb init --api-key <key> --user-id <id>");
                console.log("    openclaw memb init --email <email>");
                console.log("    openclaw memb init --email <email> --code <c>");
                console.log("  Open Source:");
                console.log("    openclaw memb init --mode open-source --oss-llm ollama --oss-embedder ollama --oss-vector qdrant");
                console.log("    openclaw memb init --mode open-source --oss-llm openai --oss-llm-key <key>");
              ...
              if (hasExistingConfig) {
                console.log("\n  Existing MemB configuration found:\n");
                ...
                if (
                  ...
                  console.log(
                    "  To update individual settings: openclaw memb config set <key> <value>\n",
                  );
                  return;
                }
                console.log("");
              ...
              console.log("\n  MemB Setup\n");
              console.log("  How would you like to use MemB?");
              console.log("  1. Platform (recommended) — hosted memory, managed by MemB");
              ...
              if (modeChoice === "1") {
                ...
                if (check.ok) {
                  console.log(
                    "  API key validated. Connected to MemB Platform.",
                  );
                } else if (check.status) {
                  console.warn(
                ...
                console.log(
                  ...
                  console.log("Invalid choice. Run `openclaw memb init` again.");
                }
      ...
      // ====================================================================
      // search (matches: memb search <query> --top-k --user-id --agent-id)
      // ====================================================================
      ...
      // ====================================================================
      // add (matches: memb add <text> --user-id --agent-id)
      // ====================================================================
      ...
      // ====================================================================
      // get (matches: memb get <memory_id>)
      // ====================================================================
      ...
      // ====================================================================
      // list (matches: memb list --user-id --agent-id --top-k)
      // ====================================================================
      ...
      // ====================================================================
      // update (matches: memb update <memory_id> <text>)
      // ====================================================================
      ...
      // ====================================================================
      // delete (matches: memb delete <memory_id> --all --user-id)
      // ====================================================================
      ...
      memb
        ...
        .action(async (opts: { json?: boolean } = {}) => {
          try {
            ...
            if (result.connected) {
              console.log("Connected to MemB");
            } else {
              console.log("Not connected to MemB");
            }
            if (result.url) {
      ...
      // ====================================================================
      // config (matches: memb config show, memb config get, memb config set)
      // ====================================================================

      const configCmd = memb
        .command("config")
      ...
      function getConfigValue(field: string): unknown {
        ...
        const values: Record<string, unknown> = {
          ...
          baseUrl: auth.baseUrl ?? cfg.baseUrl ?? "https://api.memb.ai",
      ...
      configCmd
        ...
        .action((opts: { json?: boolean } = {}) => {
          ...
          console.log("    openclaw memb config set <key> <value>");
          console.log("");
          console.log("  Examples:");
          if (cfg.mode === "platform") {
            console.log("    openclaw memb config set mode open-source");
            console.log("    openclaw memb config set auto_recall false");
          } else {
            console.log("    openclaw memb config set vector_provider qdrant");
            console.log("    openclaw memb config set llm_model gpt-4o");
            console.log("    openclaw memb config set embedder_provider openai");
          }
          console.log("");
        });
      ...
      // ====================================================================
      // import (matches: memb import <file>)
      // ====================================================================
      ...
      // ====================================================================
      // event (matches: memb event list, memb event status <id>)
      // ====================================================================
      ...
      // ====================================================================
      // help (matches: memb help, memb help --json)
      // ====================================================================
      ...
      memb
        ...
        .action((opts: { json?: boolean }) => {
          const commands = {
            ...
            management: {
              init: "Interactive setup wizard for memb CLI",
              status: "Check connectivity and authentication",
              config: "Manage memb configuration (show, get, set)",
          ...
          if (opts.json) {
            const detailed = {
              commands: {
                ...
                management: {
                  init: { description: "Set up MemB", flags: { "--mode <m>": "platform or open-source", "--api-key <k>": "API key", "--email <e>": "Email login", "--oss-llm <p>": "LLM provider", "--oss-embedder <p>": "Embedder", "--oss-vector <p>": "Vector store", "--json": "JSON output" } },
          ...
          console.log("");
          console.log("  openclaw memb <command>");
          console.log("");
          console.log("  Memory:");
    ...
    {
      descriptors: [
        { name: "memb", description: "MemB memory plugin commands", hasSubcommands: true },
      ],
    },
  );
}
