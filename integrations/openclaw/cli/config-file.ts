/**
 * File-based config helpers for the OpenClaw MemB plugin.
 *
 * Plugin auth and settings are stored in ~/.openclaw/openclaw.json under
 * plugins.entries.openclaw-memb.config — the single source of truth.
 *
...
function readFullConfig(): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(
      `[openclaw-memb] Failed to parse ${OPENCLAW_CONFIG_FILE}: ${msg}\n` +
