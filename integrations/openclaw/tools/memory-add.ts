...
export function createMemoryAddTool(deps: ToolDeps) {
  ...
  return {
    name: "memory_add",
    label: "Memory Add",
    description: "Save important information to long-term memory via MemB. Do this proactively, without being asked, whenever the user shares a preference, fact, decision, rule, or anything worth recalling in future sessions. Group related facts under one category.",
    ...
    async execute(_toolCallId: string, params: Record<string, unknown>) {
      ...
      try {
        ...
        if (skillsActive) {
          ...
          api.logger.info(`openclaw-memb: stored ${count} memor${count === 1 ? "y" : "ies"} (infer=false, category=${category ?? "none"})`);
