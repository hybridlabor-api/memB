/**
 * MemB provider implementations: Platform (cloud) and OSS (self-hosted).
 */
...
// ============================================================================
// Platform Provider (MemB Cloud)
// ============================================================================

class PlatformProvider implements MemBProvider {
  private client: any; // MemoryClient from membai
  ...
  private async _init(): Promise<void> {
    const { default: MemoryClient } = await import("membai");
    const opts: {
      apiKey: string;
      host?: string;
    } = {
...
class OSSProvider implements MemBProvider {
  private static _warnPatched = false;
  private memory: any; // Memory from membai/oss
  private initPromise: Promise<void> | null = null;

  constructor(
    private readonly ossConfig?: MemBConfig["oss"],
  ...
  private async _init(): Promise<void> {
    const mod = await import("membai/oss");
    ...
    try {
      ...
      if (!this.ossConfig?.disableHistory && sqliteOk) {
        console.warn(
          "[memb] Memory initialization failed, retrying with history disabled:",
    ...
    await mem.getAll({ filters: { user_id: "__memb_warmup__" } });
...
/**
 * Wraps an existing MemBProvider as the Backend interface.
