/**
 * Tests for config.ts — membConfigSchema.parse() and exported constants.
 */
...
// ---------------------------------------------------------------------------
// membConfigSchema.parse() — defaults
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — defaults", () => {
  it("mode defaults to 'platform' when omitted", () => {
    const cfg = membConfigSchema.parse({ apiKey: "test-key" });
  ...
  it("customPrompt input falls back to customInstructions", () => {
    const cfg = membConfigSchema.parse({ apiKey: "test-key", customPrompt: "My prompt" });
  ...
  it("allows anonymousTelemetryId", () => {
    const cfg = membConfigSchema.parse({ apiKey: "test-key", anonymousTelemetryId: "123" });
    expect(cfg.anonymousTelemetryId).toBe("123");
  });
});

// ---------------------------------------------------------------------------
// membConfigSchema.parse() — mode parsing
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — mode parsing", () => {
  it('"oss" is not a valid mode and defaults to "platform"', () => {
    const cfg = membConfigSchema.parse({ mode: "oss", apiKey: "k" });
    expect(cfg.mode).toBe("platform");
  });

  it('"open-source" stays as "open-source"', () => {
    const cfg = membConfigSchema.parse({ mode: "open-source" });
    expect(cfg.mode).toBe("open-source");
  });

  it("any other string defaults to 'platform'", () => {
    const cfg = membConfigSchema.parse({ mode: "something-else", apiKey: "k" });
    expect(cfg.mode).toBe("platform");
  });

  it("undefined mode defaults to 'platform'", () => {
    const cfg = membConfigSchema.parse({ apiKey: "k" });
    expect(cfg.mode).toBe("platform");
  });

  it("numeric mode defaults to 'platform'", () => {
    const cfg = membConfigSchema.parse({ mode: 42, apiKey: "k" });
    expect(cfg.mode).toBe("platform");
  });
});

// ---------------------------------------------------------------------------
// membConfigSchema.parse() — userId precedence
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — userId", () => {
  it("userId from config takes precedence over os.userInfo() fallback", () => {
    const cfg = membConfigSchema.parse({
      apiKey: "test-key",
      userId: "custom-user",
    });
  ...
  it("empty string userId falls back to os.userInfo()", () => {
    const cfg = membConfigSchema.parse({ apiKey: "test-key", userId: "" });
  ...
  it("non-string userId falls back to os.userInfo()", () => {
    const cfg = membConfigSchema.parse({ apiKey: "test-key", userId: 123 });
    expect(typeof cfg.userId).toBe("string");
    expect(cfg.userId.length).toBeGreaterThan(0);
  });
});
...
// ---------------------------------------------------------------------------
// membConfigSchema.parse() — needsSetup
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — needsSetup", () => {
  // Note: needsSetup = (mode === "platform" && !resolvedApiKey).
  // resolvedApiKey can come from the config OR from ~/.memb/config.json fallback.
  // When no apiKey is provided and no config file exists, needsSetup is true.
  // When ~/.memb/config.json has a key, the fallback populates resolvedApiKey.

  it("needsSetup is consistent: false when apiKey resolves, true otherwise (no apiKey in config)", () => {
    const cfg = membConfigSchema.parse({ mode: "platform" });
    // needsSetup should be true only if NO apiKey was resolved (including from ~/.memb/config.json)
    if (cfg.apiKey) {
  ...
  it("is false when apiKey is explicitly provided", () => {
    const cfg = membConfigSchema.parse({ apiKey: "my-api-key" });
    expect(cfg.needsSetup).toBe(false);
  });
  ...
  it("needsSetup is always false when apiKey is a valid string", () => {
    const cfg = membConfigSchema.parse({ apiKey: "test-key-123" });
...
// ---------------------------------------------------------------------------
// membConfigSchema.parse() — error cases
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — error cases", () => {
  it("throws on unknown keys", () => {
    expect(() =>
      membConfigSchema.parse({ apiKey: "k", unknownKey: "value" }),
    ).toThrow(/unknown keys.*unknownKey/);
  });

  it("throws when multiple unknown keys are present", () => {
    expect(() =>
      membConfigSchema.parse({ apiKey: "k", foo: 1, bar: 2 }),
    ).toThrow(/unknown keys/);
  });

  it("throws on null input", () => {
    expect(() => membConfigSchema.parse(null)).toThrow(
      "openclaw-memb config required",
    );
  });

  it("throws on undefined input", () => {
    expect(() => membConfigSchema.parse(undefined)).toThrow(
      "openclaw-memb config required",
    );
  });

  it("throws on string input", () => {
    expect(() => membConfigSchema.parse("not an object")).toThrow(
      "openclaw-memb config required",
    );
  });

  it("throws on number input", () => {
    expect(() => membConfigSchema.parse(42)).toThrow(
      "openclaw-memb config required",
    );
  });

  it("throws on array input", () => {
    expect(() => membConfigSchema.parse([1, 2, 3])).toThrow(
      "openclaw-memb config required",
    );
  });

  it("throws on boolean input", () => {
    expect(() => membConfigSchema.parse(true)).toThrow(
      "openclaw-memb config required",
    );
  });
});
...
// ---------------------------------------------------------------------------
// membConfigSchema.parse() — explicit overrides
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — explicit overrides", () => {
  ...
  it("custom topK is used when provided", () => {
    const cfg = membConfigSchema.parse({ apiKey: "k", topK: 20 });
...
// ---------------------------------------------------------------------------
// membConfigSchema.parse() — oss config
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — oss config", () => {
  it("parses oss object when provided", () => {
    ...
    const cfg = membConfigSchema.parse({ mode: "open-source", oss: ossConfig });
    expect(cfg.mode).toBe("open-source");
    expect(cfg.oss).toEqual(ossConfig);
  });

  it("ignores oss when it is not a plain object", () => {
    const cfg = membConfigSchema.parse({ mode: "open-source", oss: "not-an-object" });
    expect(cfg.oss).toBeUndefined();
  });

  it("ignores oss when it is an array", () => {
    const cfg = membConfigSchema.parse({ mode: "open-source", oss: [1, 2, 3] });
    expect(cfg.oss).toBeUndefined();
  });

  it("ignores oss when it is null", () => {
    const cfg = membConfigSchema.parse({ mode: "open-source", oss: null });
    expect(cfg.oss).toBeUndefined();
  });
});

// ---------------------------------------------------------------------------
// membConfigSchema.parse() — skills config
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — skills config", () => {
  ...
  it("skills is undefined when set to a non-object value", () => {
    const cfg = membConfigSchema.parse({ apiKey: "k", skills: "invalid" });
    expect(cfg.skills).toBeUndefined();
  });

  it("skills is undefined when set to an array", () => {
    const cfg = membConfigSchema.parse({ apiKey: "k", skills: [1, 2] });
    expect(cfg.skills).toBeUndefined();
  });

  it("skills is undefined when set to null", () => {
    const cfg = membConfigSchema.parse({ apiKey: "k", skills: null });
    expect(cfg.skills).toBeUndefined();
  });
});

// ---------------------------------------------------------------------------
// membConfigSchema.parse() — customCategories edge cases
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — customCategories edge cases", () => {
...
// ---------------------------------------------------------------------------
// membConfigSchema.parse() — non-string apiKey
// ---------------------------------------------------------------------------
describe("membConfigSchema.parse() — apiKey edge cases", () => {
  // Note: When a non-string apiKey is provided, the parser treats it as
  // undefined. However, readMemBConfigFile() may still provide a fallback
  // apiKey from ~/.memb/config.json if one exists on the system.

  it("non-string apiKey is not used directly from config", () => {
    const cfg = membConfigSchema.parse({ apiKey: 12345 });
  ...
  it("boolean apiKey is not used directly from config", () => {
    const cfg = membConfigSchema.parse({ apiKey: true });
    expect(cfg.apiKey).not.toBe(true);
  });

  it("string apiKey takes precedence over any fallback", () => {
    const cfg = membConfigSchema.parse({ apiKey: "explicit-key" });
