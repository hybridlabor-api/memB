/**
 * OpenClaw Memory (MemB) Plugin
 *
 * Long-term memory via MemB — supports both the MemB platform
 * and the open-source self-hosted SDK. Uses the official `membai` package.
 *
 * Features:
 ...
 * - CLI: openclaw memb search, openclaw memb status
...
import { membConfigSchema } from "./config.ts";
...
export { membConfigSchema } from "./config.ts";
...
const memoryPlugin = definePluginEntry({
  id: "openclaw-memb",
  name: "Memory (MemB)",
  description: "MemB memory backend — MemB platform or self-hosted open-source",

  register(api: OpenClawPluginApi) {
    bootstrapTelemetryFlag();

    // Read auth from openclaw.json plugin config (picks up post-startup login).
    // This is the single source of truth — set via `openclaw memb login`.
    const pluginAuth = readPluginAuth();
    const fileConfig: FileConfig = {
      apiKey: pluginAuth.apiKey,
      baseUrl: pluginAuth.baseUrl,
    };
    const cfg = membConfigSchema.parse(api.pluginConfig, fileConfig);
    ...
    if (cfg.needsSetup) {
      api.logger.warn(
        "openclaw-memb: API key not configured. Memory features are disabled.\n" +
          "  To set up, run:\n" +
          "  openclaw memb init\n" +
          "  Get your key at: https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=openclaw-src",
      );
      ...
      api.registerService({
        id: "openclaw-memb",
        start: () => {
          api.logger.info("openclaw-memb: waiting for API key configuration");
        },
        stop: () => {},
      });
      return;
    ...
    if (cfg.mode === "platform") {
      backend = new PlatformBackend({
        apiKey: cfg.apiKey!,
        baseUrl: cfg.baseUrl ?? "https://api.memb.ai",
      });
    } else {
    ...
    api.logger.info(
      `openclaw-memb: registered (mode: ${cfg.mode}, user: ${cfg.userId}, autoRecall: ${cfg.autoRecall}, autoCapture: ${cfg.autoCapture}, skills: ${skillsActive})`,
    );
    ...
    if (typeof api.registerMemoryCapability === "function") {
        ...
        runtime: {
          async getMemorySearchManager(_params: any) {
            try {
              ...
              return {
                manager: null,
                error: `memb ${cfg.mode} backend unavailable: ${String(err)}`,
              };
            }
          },
      ...
      api.logger.debug("openclaw-memb: memory capability + runtime registered");
    }
    ...
    function buildAddOptions(
      ...
      // Pass customInstructions and customCategories to control what MemB extracts
    ...
    api.registerService({
      ...
      start: (...args: any[]) => {
        ...
        api.logger.info(
          `openclaw-memb: initialized (mode: ${cfg.mode}, user: ${cfg.userId}, autoRecall: ${cfg.autoRecall}, autoCapture: ${cfg.autoCapture}, stateDir: ${pluginStateDir ?? "none"})`,
        );
      },
      stop: () => {
        api.logger.info("openclaw-memb: stopped");
      },
    });
  },
});
...
function registerHooks(
  ...
  if (skillsActive) {
    ...
    api.on("before_prompt_build", async (event: any, ctx: any) => {
      ...
      if (isNonInteractiveTrigger(trigger, sessionId)) {
        api.logger.info(
          "openclaw-memb: skills-mode skipping non-interactive trigger",
        );
        return;
      }
      ...
      if (isSystemPrompt) {
        api.logger.info(
          "openclaw-memb: skills-mode skipping recall for system/bootstrap prompt",
        );
      ...
      if (recallEnabled && recallStrategy !== "manual") {
        ...
        try {
          ...
          api.logger.info(
            `openclaw-memb: skills-mode recall (strategy=${recallStrategy}) injecting ${recallResult.memories.length} memories (~${recallResult.tokenEstimate} tokens)`,
          );
          ...
          api.logger.warn(
            `openclaw-memb: skills-mode recall failed: ${String(err)}`,
          );
        }
      } else if (recallEnabled && recallStrategy === "manual") {
        api.logger.info(
          "openclaw-memb: skills-mode recall strategy=manual, agent controls search",
        );
      }
      ...
      if (dreamEnabled && stateDir && !isSubagent) {
        try {
          ...
          if (cheapResult.proceed) {
            ...
            if (memResult.pass && acquireDreamLock(stateDir)) {
              ...
              if (dreamPrompt) {
                ...
                api.logger.info(
                  `openclaw-memb: auto-dream triggered (${memCount} memories, gate passed)`,
                );
              } else {
          ...
          api.logger.warn(
            `openclaw-memb: auto-dream gate check failed: ${String(err)}`,
          );
        }
      }
    ...
    api.on("agent_end", async (event: any, ctx: any) => {
      ...
      if (dreamSessionId && dreamSessionId === sessionId && stateDir) {
        ...
        if (!event.success) {
          ...
          releaseDreamLock(stateDir);
          api.logger.warn(
            "openclaw-memb: auto-dream turn failed, lock released, will retry",
          );
          return;
        }
        ...
        if (writeToolUsed) {
          ...
          api.logger.info(
            "openclaw-memb: auto-dream completed (verified write tool usage), lock released",
          );
        } else {
          releaseDreamLock(stateDir);
          api.logger.warn(
            "openclaw-memb: auto-dream injected but no write tools executed. Lock released, will retry.",
          );
        }
        return;
      }
      ...
      api.logger.info("openclaw-memb: skills-mode agent_end (no auto-capture)");
    });
  ...
  if (cfg.autoRecall) {
    ...
    api.on("before_prompt_build", async (event: any, ctx: any) => {
      ...
      if (isNonInteractiveTrigger(trigger, sessionId)) {
        api.logger.info(
          "openclaw-memb: skipping recall for non-interactive trigger",
        );
        return;
      }
      ...
      if (isSystemPrompt) {
        api.logger.info(
          "openclaw-memb: skipping recall for system/bootstrap prompt",
        );
        return;
      }
      ...
      const recallWork = async () => {
        ...
        api.logger.info(
          `openclaw-memb: injecting ${longTermResults.length} memories into context`,
        );
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          timeout.then(() => {
            api.logger.warn(
              `openclaw-memb: recall timed out after ${RECALL_TIMEOUT_MS}ms, skipping`,
            );
            return undefined;
          }),
        ]);
        ...
        api.logger.warn(`openclaw-memb: recall failed: ${String(err)}`);
      }
    });
  }
  ...
  if (cfg.autoCapture) {
    api.on("agent_end", async (event, ctx) => {
      ...
      if (isNonInteractiveTrigger(trigger, sessionId)) {
        api.logger.info(
          "openclaw-memb: skipping capture for non-interactive trigger",
        );
        return;
      }
      ...
      if (isSubagentSession(sessionId)) {
        api.logger.info(
          "openclaw-memb: skipping capture for subagent (main agent captures consolidated result)",
        );
        return;
      }
      ...
      if (agentUsedMemoryTool) {
        api.logger.info(
          "openclaw-memb: skipping auto-capture — agent already used memory tools this turn",
        );
        return;
      }
      ...
      if (userContent.length < 50) {
        api.logger.info(
          "openclaw-memb: skipping capture — user content too short for meaningful extraction",
        );
        return;
      }
      ...
      provider
        ...
        .then((result) => {
          ...
          if (capturedCount > 0) {
            api.logger.info(
              `openclaw-memb: auto-captured ${capturedCount} memories`,
            );
          }
        })
        .catch((err) => {
          api.logger.warn(`openclaw-memb: capture failed: ${String(err)}`);
        });
    });
  }
}
