...
Long-term memory for [OpenClaw](https://github.com/openclaw/openclaw) agents, powered by [MemB](https://memb.ai).
...
By default, the plugin runs in **skills mode**: the agent controls what to remember (triage), how to recall (recall), and periodic cleanup (dream). Skills mode, `autoRecall`, and `autoCapture` are all enabled by default during `openclaw memb init`.
...
If you're an AI agent setting up MemB autonomously (no human dashboard signup), mint an evaluation API key in <5 seconds:
...
1. **Install the plugin via the OpenClaw CLI:**
   ...
   ```bash
   openclaw plugins install @memb/openclaw-memb
   ```

2. **Get your API key** from [app.memb.ai](https://app.memb.ai/dashboard/api-keys?utm_source=oss&utm_medium=openclaw-readme).

3. **Select the plugin as your memory backend in `openclaw.json`.** Either initialize via the CLI:

   ```bash
   openclaw memb init --api-key <your-key> --user-id <your-user-id>
   ```
...
```bash
openclaw plugins update openclaw-memb
```

### Open-Source (Self-hosted)

No MemB key needed. Vectors are stored locally in SQLite at `~/.memb/vector_store.db` — no external database required.
...
```bash
openclaw memb init --mode open-source
```
...
openclaw memb init --mode open-source --oss-llm ollama --oss-embedder ollama --oss-vector qdrant --json
```
...
All `oss` fields are optional. See the [MemB OSS docs](https://docs.memb.ai/open-source/node-quickstart) for supported providers.

## How It Works

<p align="center">
  <img src="https://raw.githubusercontent.com/membai/memb/main/docs/images/openclaw-architecture.png" alt="Architecture" width="800" />
</p>

### Skills Mode (Default)

Enabled automatically during `openclaw memb init`. The agent controls memory through three skills:
...
- **Auto-Recall** — Before the agent responds, the plugin searches MemB for relevant memories and injects them into context.
- **Auto-Capture** — After the agent responds, the conversation is filtered through a noise-removal pipeline and sent to MemB. New facts get stored, stale ones updated, duplicates merged.
...
All commands: `openclaw memb <command>`. All commands support `--json` for machine-readable output (for LLM agents).

```bash
# Memory operations
openclaw memb add "User prefers TypeScript over JavaScript"
openclaw memb search "what languages does the user know"
openclaw memb search "preferences" --scope long-term
openclaw memb get <memory_id>
openclaw memb list --user-id alice --top-k 20
openclaw memb update <memory_id> "Updated preference text"
openclaw memb delete <memory_id>
openclaw memb delete --all --user-id alice --confirm
openclaw memb import memories.json

# Management
openclaw memb init                                          # interactive setup
openclaw memb init --mode open-source --oss-llm ollama      # non-interactive OSS
openclaw memb init --api-key <key> --user-id alice          # non-interactive platform
openclaw memb status
openclaw memb config show
openclaw memb config get api_key
openclaw memb config set user_id alice

# Events (platform only)
openclaw memb event list
openclaw memb event status <event_id>

# Memory consolidation
openclaw memb dream
openclaw memb dream --dry-run

# JSON output (any command)
openclaw memb search "preferences" --json
openclaw memb list --json
openclaw memb status --json
openclaw memb help --json                                   # discover all commands + flags
```
...
Enabled by default during `openclaw memb init`. `autoRecall` and `autoCapture` are also `true` by default and work alongside skills mode.
...
All fields optional. Defaults: `text-embedding-3-small` embeddings, local SQLite vector store (`~/.memb/vector_store.db`), `gpt-5-mini` LLM.
...
In **skills mode** (default after `openclaw memb init`), the agent uses structured protocols (triage, recall, dream) to decide what to store and recall. The built-in `session-memory` hook is disabled to avoid conflicts.
...
In platform mode, conversation content is sent to `api.memb.ai` for processing. Do not use with sensitive data you do not want stored on MemB cloud.
...
| `~/.memb/vector_store.db` | Local vector store (open-source mode only) |
| `~/.memb/history.db` | Memory edit history (open-source mode only) |
