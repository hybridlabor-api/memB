...
import type { MemBConfig, MemBMode } from "./types.ts";
...
export const membConfigSchema = {
  parse(value: unknown, fileConfig?: FileConfig): MemBConfig {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      throw new Error("openclaw-memb config required");
    }
    const cfg = value as Record<string, unknown>;
    assertAllowedKeys(cfg, ALLOWED_KEYS, "openclaw-memb config");
    ...
    if (
      ...
      console.warn(
        `[memb] Unknown mode "${cfg.mode}" — expected "platform" or "open-source". Defaulting to "platform".`,
      );
    }
    const mode: MemBMode =
    ...
    let ossConfig: MemBConfig["oss"];
    if (cfg.oss && typeof cfg.oss === "object" && !Array.isArray(cfg.oss)) {
      ossConfig = cfg.oss as MemBConfig["oss"];
    }
    ...
    return {
      ...
      skills:
        ...
        !Array.isArray(cfg.skills)
          ? (cfg.skills as MemBConfig["skills"])
          : undefined,
    };
  },
};
